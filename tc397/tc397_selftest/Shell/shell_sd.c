...
/* Memory placement                                                    */
/* ------------------------------------------------------------------ */
/* The bulk buffers of this file (I/O staging, mkfs work area, big-block
 * test buffer) are far too large for DSPR0: the combined tc397_selftest
 * firmware needs most of the 240 KB DSPR0 for the lwIP heap and the
 * protocol stacks.  They are therefore placed in LMU (768 KB, see the
 * "LMU Data sections" group in Lcf_*_Tricore_Tc.lsl).
 * LMU is also reachable by the SDMMC ADMA2 engine and disk_read_sdmmc() /
 * disk_write_sdmmc() already run the required cache maintenance
 * (sdmmc_cache_clean = CACHEA.WI + DSYNC) for any buffer address, so the
 * cached LMU view is safe for DMA here.
 * NOTE: the pragma must be closed again before the SHELL_EXPORT_CMD table,
 * because TriCore-GCC applies the default section to const objects too and
 * then rejects the explicit section("shellCommand") attribute. */
...
/* Large single-transfer buffer for `sd big` / `sd erase`: 48 KB = 96 blocks.
 * Still larger than the 64-entry-per-page ADMA2 table, so the chained
 * descriptor path (link hops) is exercised end to end; the previous 320
 * block (160 KB) size did not fit next to the rest of the combined firmware. */
#define SD_BIG_BLOCKS   96U
