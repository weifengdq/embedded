...
/* ---------- Combined self-test / bench / status ---------- */
static int cmd_selftest(int argc, char *argv[])
{
    ...
    if (argc >= 2) {
        if (strcmp(argv[1], "help") == 0) {
            shellPrint(shell, "usage: selftest [core|uart|adc|can|fr|tlf|sd|t1s|geth|net|all]\r\n");
            shellPrint(shell, "  selftest        automatic factory check over all peripherals\r\n");
            shellPrint(shell, "  selftest all    same, with the detail/evidence lines\r\n");
            shellPrint(shell, "  selftest <key>  run one group only\r\n");
            shellPrint(shell, "  stat            live peripheral status summary (no test)\r\n");
            shellPrint(shell, "  bench           on-board throughput measurements\r\n");
            return 0;
        }
        if (strcmp(argv[1], "stat") == 0) {
            Selftest_Status(shell);
            return 0;
        }
        if (strcmp(argv[1], "all") == 0) {
            Selftest_Run(shell, 1);
            return 0;
        }
    }
    Selftest_Run(shell, (argc >= 2) ? 1 : 0);
    return 0;
}

static int cmd_bench(int argc, char *argv[])
{
    Selftest_Bench(shellGetCurrent(), argc, argv);
    return 0;
}
...
{
    (void)argc; (void)argv;
    Selftest_Status(shellGetCurrent());
    return 0;
}
...
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN), selftest, cmd_selftest, factory self-test over all peripherals);
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN), stat, cmd_stat, live peripheral status summary);
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN), bench, cmd_bench, on-board throughput bench);
