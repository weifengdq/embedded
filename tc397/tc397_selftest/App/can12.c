...
/* g_can is ~84 KB zero-init. In the combined tc397_selftest firmware DSPR0
 * (240 KB) is almost entirely taken by the lwIP heap and the protocol
 * stacks, so the 12-channel driver state is placed in LMU (768 KB, see the
 * "LMU Data sections" group in Lcf_*_Tricore_Tc.lsl) - the same place the
 * GETH DMA buffers live. The CAN message RAM itself is NOT here: it sits in
 * the dedicated 0xF0200000 area, so this array is plain CPU data and LMU is
 * a fine home for it.
 * Forcing a NOBITS section also keeps ~84 KB of flash LMA copy out of the
 * image (all fields are assigned at init + rxRing is memset, so the
 * behaviour is deterministic either way). */
