...
/* Multi-core spinlock in LMURAM (cleared to 0 by Ssw, no flash copy).
 * NOTE: keep it in a NOBITS section: a PROGBITS ".lmudata" section gets a
 * Ssw copy-table entry whose flash LMA can be odd-aligned (Lcf ALIGN(2)),
 * while the Ssw C-init performs word-sized copies. */
volatile uint32 lwip_global_lock;
...
void Ifx_Lwip_pollReceiveFlags(void)
{
    ...
    /* Second netif (LAN8651 10BASE-T1S): link polling + RX drain. Runs here
     * because this function is the only main-loop poll with interrupts
     * enabled (the QSPI4 driver is ISR driven). */
    IfxLan8651_poll();
...
{
    /* NOTE (tc397_selftest): the original port calls initUART() here to make
     * the debug console available for stand-alone use.  The application has
     * already initialised the console (and printed the boot banner), and a
     * second IfxAsclin_Asc_initModule() in the middle of the boot sequence
     * re-initialises the TX/RX rings, which chopped the banner and the
     * following Ifx_Lwip_printf() output into each other.  Left out here. */
...
{
    /* See the note in Ifx_Lwip_init(): no second initUART() here. */
