    ...
    const uint8_t *mac_addr)
{
    ...
    masterConfig.txPriority = ISR_PRIORITY_QSPI4_TX;
    masterConfig.rxPriority = ISR_PRIORITY_QSPI4_RX;
    masterConfig.erPriority = ISR_PRIORITY_QSPI4_ER;
...
lan8651_status_t lan8651_transmit(lan8651_t *dev, const uint8_t *frame, uint16_t len)
{
    ...
    for (i = 0U; i < chunks; i++)
    {
        ...
        *(uint32_t *)&dev->tx_buf[0] = lan8651_bswap32(lan8651_make_data_header(is_first, 0U, is_last, ebo, 1U));
        ...
        {
            lan8651_status_t st = lan8651_spi_transfer(dev, LAN8651_TC6_DATA_BUF_SIZE);
            if (st != kLan8651Status_Ok)
            {
