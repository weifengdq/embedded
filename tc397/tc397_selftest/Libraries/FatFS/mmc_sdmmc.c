...
DRESULT disk_write_sdmmc(BYTE pdrv, const BYTE *buff, LBA_t sector, UINT count)
{
    ...
    /* Write-back + invalidate the source buffer before the ADMA2 engine reads
     * it.  The standalone tc397_sdmmc project kept its buffers in DSPR (which
     * is not cached) and therefore did not need this; tc397_selftest places
     * them in cached LMU to save DSPR0, and without the clean the DMA would
     * fetch stale data from LMU and every read-back verify failed.
     * CACHEA.WI is a no-op for non-cached (DSPR) addresses, so this is safe
     * for both placements. */
    sdmmc_cache_clean((const void *)buff, (uint32_t)count * 512U);
