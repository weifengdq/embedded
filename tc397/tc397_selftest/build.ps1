<#
.SYNOPSIS
    tc397_selftest CMake 命令行构建脚本（TriCore GCC + TASKING 双工具链）。
...
.PARAMETER AurixStudioPath
    ...
    默认: C:\Infineon\AURIX-Studio-1.10.36
...
param(
    ...
    [string]$TargetName = "tc397_selftest",

    # ---- 高层路径（推导 GCC/TASKING/Flasher/make 子路径）----
    [string]$AurixStudioPath = "C:\Infineon\AURIX-Studio-1.10.36",
...
function Invoke-Build {
    ...
    Remove-StaleNinjaDeps
    Invoke-Step -FilePath "cmake" -ArgumentList @(
        "--build", $buildPath, "--config", $BuildType, "--", "-j$logicalCores"
    )
}

# ninja 的依赖数据库 .ninja_deps 在本工程树上不可复用：
# TASKING 的 --dep-file 每行写成 "target : dep" 的 make 风格对（同一路径既作 target
# 又作 dep），ninja 解析后会把一个截断的目录路径（如 .../App 或
# .../Configurations/Debug）写进 .ninja_deps。下次 ninja 启动 stat 该路径时直接
# 报错退出：
#   ninja: error: FindFirstFileExA(".../App): The filename, directory name, or
#   volume label syntax is incorrect.
# 与 ninja 版本无关（PyPI 1.13.0、AURIX Studio 1.13.2、WinGet 1.13.2 均复现）。
# 删掉 .ninja_deps 即可让增量构建恢复正常；代价是头文件改动不再自动触发重编，
# 因此改头文件后请用 -Action rebuild。
function Remove-StaleNinjaDeps {
    $deps = Join-Path $buildPath ".ninja_deps"
    if (Test-Path $deps) {
        Write-Host ("> Remove-Item -Force {0}" -f $deps)
        Remove-Item -Force $deps
    }
}
