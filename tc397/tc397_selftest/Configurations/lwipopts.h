...
/* ---- Checksum handling (two netifs, different capabilities) ----------------
 * GETH does checksum insertion/verification in the MAC (MAC_CONFIGURATION.IPC
 * plus TDES3.CIC_TPL = 3 in netif.c), so the GETH netif keeps the lwIP default
 * "NETIF_CHECKSUM_DISABLE_ALL" that netif_add() applies.
 * The LAN8651 (10BASE-T1S) has no checksum offload, so its netif is switched to
 * NETIF_CHECKSUM_ENABLE_ALL and lwIP computes/verifies the checksums in soft-
 * ware (affordable at 10 Mbit/s).
 * LWIP_CHECKSUM_CTRL_PER_NETIF=1 turns the per-netif control on, which is why
 * the CHECKSUM_GEN_xxx / CHECKSUM_CHECK_xxx macros must stay at 1 here (they
 * now only gate the runtime flag check, they no longer force all netifs). */
