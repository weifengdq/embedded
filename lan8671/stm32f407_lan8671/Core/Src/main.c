...
/* USER CODE BEGIN PFP */
static void ConfigureRmiiReferenceClockOutput(void);
...
static void ConfigureRmiiReferenceClockOutput(void)
{
  ...
  periph_clk_init.PeriphClockSelection = RCC_PERIPHCLK_PLLI2S;
  periph_clk_init.PLLI2S.PLLI2SN = 100;
  periph_clk_init.PLLI2S.PLLI2SR = 2;
...
int main(void)
{
  ...
  ConfigureRmiiReferenceClockOutput();
  ...
  MX_LWIP_Init();
  debug_printf("\r\nSTM32F407 + LAN8671 startup\r\n");
  App_Network_Init();
  App_Network_OnLinkStatusChanged(&gnetif);
