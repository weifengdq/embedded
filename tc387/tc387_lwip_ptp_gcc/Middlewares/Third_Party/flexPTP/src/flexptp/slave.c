...
void ptp_slave_tick() {
    ...
    uint64_t masterClockId = ptp_get_current_master_clock_identity();
    bool masterKnown = (masterClockId != 0U) && (masterClockId != ptp_get_own_clock_identity());

    // Delay_Req transmission
    if (masterKnown && (S.profile.logDelayReqPeriod != PTP_LOGPER_SYNCMATCHED)) {
