...
param(
    ...
    [string]$ProjectName = 'h503_sja1124',
...
function Invoke-CMakeConfigure([string]$presetName) {
    ...
    # Use --fresh if supported (CMake >= 3.24)
...
Action:
  build      配置 + 编译 (默认)
  clean      清理当前 preset (优先 clean target, 否则删目录)
  distclean  删除 build 目录下当前 preset 输出
  rebuild    clean + build
  gen        从 .elf 生成 .hex/.bin
  flash      gen + 用 STM32CubeProgrammer 下载
  help       显示帮助
...
示例:
  ...
  .\build.ps1 flash Debug -FlashFormat bin -FlashAddress 0x08000000

依赖:
  - cmake (支持 presets)
  - ninja (CMake generator 用 Ninja)
  - arm-none-eabi-gcc 工具链 (至少包含 arm-none-eabi-objcopy)
  - 下载: STM32CubeProgrammer (STM32_Programmer_CLI.exe)

提示:
  - 若执行策略阻止脚本: PowerShell 里用 `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned`
    或运行: `powershell -ExecutionPolicy Bypass -File .\build.ps1 build Debug`
"@ | Write-Host
}
...
switch ($Action) {
    ...
    'flash' {
        ...
        if (-not $cli) {
            throw "未找到 STM32CubeProgrammer CLI。请安装 STM32CubeProgrammer，或用 -CubeProgrammerCli 指定 STM32_Programmer_CLI.exe 路径。"
        }
