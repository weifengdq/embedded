...
#define APP_RTL8211_BMSR_LINK_STATUS_MASK  (0x0004U)
#define APP_RTL8211_PHYSR_DUPLEX_MASK      (0x0008U)
#define APP_RTL8211_PHYSR_SPEED_MASK       (0x0630U)
#define APP_RTL8211_PHYSR_SPEED_10         (0x0000U)
#define APP_RTL8211_PHYSR_SPEED_100        (0x0010U)
#define APP_RTL8211_PHYSR_SPEED_1000       (0x0020U)
#define APP_RTL8211_PAGE_SELECT_REG        (0x1FU)
#define APP_RTL8211F_RGMII_PAGE            (0x0D08U)
#define APP_RTL8211F_TXCR                  (0x11U)
#define APP_RTL8211F_TX_DELAY_MASK         (0x0100U)
#define APP_RTL8211F_RXCR                  (0x15U)
#define APP_RTL8211F_RX_DELAY_MASK         (0x0008U)

static uint32_t detect_rtl8211_phy_addr(ENET_Type *ptr)
{
    uint32_t phy_addr;

    for (phy_addr = 0; phy_addr < 8U; phy_addr++) {
        uint16_t id1 = enet_read_phy(ptr, phy_addr, RTL8211_PHYID1);
        uint16_t id2 = enet_read_phy(ptr, phy_addr, RTL8211_PHYID2);

        if ((RTL8211_PHYID1_OUI_MSB_GET(id1) == RTL8211_ID1)
            && (RTL8211_PHYID2_OUI_LSB_GET(id2) == RTL8211_ID2)) {
            s_rtl8211_phy_addr = phy_addr;
            printf("RTL8211 detected at MDIO address %u (ID1=0x%04X ID2=0x%04X)\n",
                   (unsigned int) phy_addr,
                   id1,
                   id2);
            if (phy_addr != RTL8211_ADDR) {
                printf("RTL8211 strap differs from configured default address %u\n",
    ...
    printf("RTL8211 not found during MDIO scan, fallback to configured address %u\n",
           (unsigned int) RTL8211_ADDR);
    s_rtl8211_phy_addr = RTL8211_ADDR;
    return s_rtl8211_phy_addr;
}

static void rtl8211_get_phy_status_local(ENET_Type *ptr, uint32_t phy_addr, enet_phy_status_t *status)
{
    uint16_t bmsr;
    uint16_t physr;
    ...
    switch (physr & APP_RTL8211_PHYSR_SPEED_MASK) {
    case APP_RTL8211_PHYSR_SPEED_10:
        status->enet_phy_speed = enet_phy_port_speed_10mbps;
        break;
    case APP_RTL8211_PHYSR_SPEED_100:
        status->enet_phy_speed = enet_phy_port_speed_100mbps;
        break;
    case APP_RTL8211_PHYSR_SPEED_1000:
    default:
...
void enet_self_adaptive_port_speed(void)
{
    ...
    status.enet_phy_duplex = MAC_TO_MAC_FIXED_LINK_DUPLEX;
        ...
        #elif defined(__USE_RTL8211) && __USE_RTL8211
        rtl8211_get_phy_status_local(ENET, s_rtl8211_phy_addr, &status);
        #endif
    #else
        #if defined(__USE_DP83848) && __USE_DP83848
