...
{
    /* ETH_NRST: PF14 as GPIO output for DP83848 hardware reset */
    HPM_IOC->PAD[IOC_PAD_PF14].FUNC_CTL = IOC_PF14_FUNC_CTL_GPIO_F_14;

    /* MDIO: pull-up required for DP83848 open-drain MDIO */
    HPM_IOC->PAD[IOC_PAD_PA30].PAD_CTL = IOC_PAD_PAD_CTL_PE_SET(1) | IOC_PAD_PAD_CTL_PS_SET(1);

    /* PF02: ETH0_TXCK with LOOP_BACK.
     * LOOP_BACK: MAC uses its own PLL2 50MHz output via internal loopback (works without wire).
     * Physical pin also outputs 50MHz.
     * --->>> ADD WIRE: HPM5E31 PF02 ---> DP83848 module REF_CLK input <<<---
     * After wire: both MAC and DP83848 share the SAME 50MHz -> 0% clock-drift loss. */
