...
#if defined(RGMII) && RGMII && defined(__USE_88Q2112) && __USE_88Q2112

/*---------------------------------------------------------------------*
 * 88Q2112 C45 MDIO Access Helpers (via C22 indirect registers 13/14)
 *---------------------------------------------------------------------*/
#define MDIO_C22_MMD_CTRL_REG   (13U)
#define MDIO_C22_MMD_DATA_REG   (14U)

#define MDIO_MMD_CTRL_FUNC_ADDR       (0x0000U)
#define MDIO_MMD_CTRL_FUNC_DATA_NOINC (0x4000U)
#define MDIO_MMD_CTRL_FUNC_DATA_RINC  (0xC000U)

#define MDIO_MMD_PMAPMD   (1U)
#define MDIO_MMD_PCS      (3U)
#define MDIO_MMD_AN       (7U)

#define MDIO_CTRL1            (0x0000U)
#define MDIO_CTRL1_LPOWER     (0x0800U)

#define MV_MMD_AN_STAT        (0x8001U)
#define MV_MMD_AN_STAT2       (0x803AU)
#define MV_MMD_AN_STAT_LOCAL_RX    (0x1000U)
#define MV_MMD_AN_STAT_REMOTE_RX   (0x2000U)
#define MV_MMD_AN_STAT2_1000BT1    (0x4000U)

/* 88Q2110 PHY ID: OUI = 0x002B */
#define Q2112_ID1  (0x002BU)
#define Q2112_ID2_MASK   (0xFFF0U)
#define Q2112_ID2_EXPECT (0x0980U)

static uint32_t s_q2112_phy_addr = Q2112_ADDR;

static void enet_c45_write(ENET_Type *ptr, uint32_t phy_addr,
                           uint8_t devad, uint16_t regnum, uint16_t val)
{
    enet_write_phy(ptr, phy_addr, MDIO_C22_MMD_DATA_REG, regnum);
    enet_write_phy(ptr, phy_addr, MDIO_C22_MMD_CTRL_REG,
                   MDIO_MMD_CTRL_FUNC_ADDR | (devad & 0x1FU));
    enet_write_phy(ptr, phy_addr, MDIO_C22_MMD_DATA_REG, val);
    enet_write_phy(ptr, phy_addr, MDIO_C22_MMD_CTRL_REG,
                   MDIO_MMD_CTRL_FUNC_DATA_NOINC | (devad & 0x1FU));
}

static uint16_t enet_c45_read(ENET_Type *ptr, uint32_t phy_addr,
                              uint8_t devad, uint16_t regnum)
{
    enet_write_phy(ptr, phy_addr, MDIO_C22_MMD_DATA_REG, regnum);
    enet_write_phy(ptr, phy_addr, MDIO_C22_MMD_CTRL_REG,
                   MDIO_MMD_CTRL_FUNC_ADDR | (devad & 0x1FU));
    enet_write_phy(ptr, phy_addr, MDIO_C22_MMD_CTRL_REG,
                   MDIO_MMD_CTRL_FUNC_DATA_RINC | (devad & 0x1FU));
    return enet_read_phy(ptr, phy_addr, MDIO_C22_MMD_DATA_REG);
}

static uint32_t detect_q2112_phy_addr(ENET_Type *ptr)
{
    uint32_t phy_addr;
...
static void q2112_get_phy_status(ENET_Type *ptr, uint32_t phy_addr,
                                  enet_phy_status_t *status)
{
    uint16_t bmsr, stat1, stat2;

    bmsr = enet_read_phy(ptr, phy_addr, 1);
    bmsr = enet_read_phy(ptr, phy_addr, 1);

    stat1 = enet_c45_read(ptr, phy_addr, MDIO_MMD_AN, MV_MMD_AN_STAT);
    stat2 = enet_c45_read(ptr, phy_addr, MDIO_MMD_AN, MV_MMD_AN_STAT2);

    status->enet_phy_link = ((bmsr & 0x0004U) != 0U) ? enet_phy_link_up : enet_phy_link_down;

    if (stat2 & MV_MMD_AN_STAT2_1000BT1) {
    ...
    if (status->enet_phy_link) {
        static uint16_t last_stat1, last_stat2;
...
static bool q2112_init(ENET_Type *ptr, uint32_t phy_addr)
{
    uint16_t val;
    ...
    enet_c45_write(ptr, phy_addr, MDIO_MMD_PCS, 0xfe34, 0x0040);
    enet_c45_write(ptr, phy_addr, MDIO_MMD_AN,  0x8032, 0x0064);
    enet_c45_write(ptr, phy_addr, MDIO_MMD_AN,  0x8031, 0x0a01);
    enet_c45_write(ptr, phy_addr, MDIO_MMD_AN,  0x8031, 0x0c01);
    enet_c45_write(ptr, phy_addr, MDIO_MMD_PCS, 0xffdb, 0x0010);

    /* Enable temp sensor, ensure not in low-power */
    enet_c45_write(ptr, phy_addr, MDIO_MMD_PCS, 0x8032, 0x6001);
    val = enet_c45_read(ptr, phy_addr, MDIO_MMD_PMAPMD, MDIO_CTRL1);
...
hpm_stat_t enet_init(ENET_Type *ptr)
{
    ...
    hpm_stat_t stat;
        ...
        #elif defined(__USE_88Q2112) && __USE_88Q2112
        /* 88Q2112 uses custom C45 init, no SDK phy_config needed */
    ...
    /* Initialize phy */
    #if defined(RGMII) && RGMII
        #if defined(__USE_88Q2112) && __USE_88Q2112
        {
        ...
        #elif defined(__USE_DP83867) && __USE_DP83867
        {
        ...
        #elif defined(__USE_RTL8211) && __USE_RTL8211
        {
            uint32_t rtl8211_phy_addr = detect_rtl8211_phy_addr(ptr);
            rtl8211_reset(ptr, rtl8211_phy_addr);
...
void enet_self_adaptive_port_speed(void)
{
    ...
    #if defined(MAC_TO_MAC_FIXED_LINK) && MAC_TO_MAC_FIXED_LINK
    status.enet_phy_link = enet_phy_link_up;
    status.enet_phy_speed = MAC_TO_MAC_FIXED_LINK_PHY_SPEED;
    status.enet_phy_duplex = MAC_TO_MAC_FIXED_LINK_DUPLEX;
    #elif defined(RGMII) && RGMII
        #if defined(__USE_DP83867) && __USE_DP83867
        dp83867_get_phy_status(ENET, DP83867_ADDR, &status);
        #elif defined(__USE_RTL8211) && __USE_RTL8211
#if defined(RGMII) && RGMII && defined(__USE_88Q2112) && __USE_88Q2112
    q2112_get_phy_status(ENET, s_q2112_phy_addr, &status);
#else
    status.enet_phy_link = enet_phy_link_down;
    status.enet_phy_speed = enet_phy_port_speed_100mbps;
    status.enet_phy_duplex = enet_phy_duplex_full;
#endif
        #endif
