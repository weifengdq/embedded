 ...
 * \param enable [in] Boolean value indicating whether to enable or disable the I2C module.
 *                  - TRUE: Enable the I2C module.
 *                  - FALSE: Disable the I2C module.
 * \return None
 */
...
/** \brief Selects the SDA line for I2C communication.
...
IFX_INLINE void IfxScrI2c_enable(boolean enable)
{
    SCR_I2CSCR_CNTR.B.ENAB = (enable == TRUE) ? 1u : 0u;
}


IFX_INLINE void IfxScrI2c_enableInterrupt(boolean enable)
{
    SCR_I2CSCR_CNTR.B.IEN = (enable == TRUE) ? 1 : 0;
}
...
IFX_INLINE void IfxScrI2c_configureSlaveAddress(uint16 slaveAddr)
{
    ...
    {
        SCR_I2CSCR_ADDR.B.SLA = (uint8)slaveAddr;
    }
    else
    {
        SCR_I2CSCR_ADDRX.B.SLAX = (uint8)slaveAddr;
        SCR_I2CSCR_ADDR.B.SLA   = ((IFXSCRI2C_10BITADDR_FIXEDBITS << 2u) | ((slaveAddr >> 8u) & 0x3u));
    }
}
...
IFX_INLINE void IfxScrI2c_enableGeneralCallAddress(boolean enable)
{
    SCR_I2CSCR_ADDR.B.GCE = (enable == TRUE) ? 1u : 0u;
}


IFX_INLINE void IfxScrI2c_reset(void)
{
    SCR_I2CSCR_SRST.B.SRST = 1u;
}


IFX_INLINE void IfxScrI2c_selectScl(IfxScrI2c_Scl sclLine)
{
    Ifx_SCR_I2CSCR_SRST srst;
    srst.U          = SCR_I2CSCR_SRST.U;
    srst.B.SCLSEL_P = 1u;
    srst.B.SCLSEL   = sclLine;
    SCR_I2CSCR_SRST.U  = srst.U;
}
...
IFX_INLINE IfxScrI2c_Status IfxScrI2c_getStatusCode(void)
{
    return (IfxScrI2c_Status)(SCR_I2CSCR_STAT.U);
}


IFX_INLINE void IfxScrI2c_assertAck(void)
{
    SCR_I2CSCR_CNTR.B.AAK = 1u;
}


IFX_INLINE void IfxScrI2c_assertNack(void)
{
    SCR_I2CSCR_CNTR.B.AAK = 0u;
}


IFX_INLINE void IfxScrI2c_clearInterrupt(void)
{
    SCR_I2CSCR_CNTR.B.IFLG = 0u;
}


IFX_INLINE boolean IfxScrI2c_isInterruptOccurred(void)
{
    return (boolean)(SCR_I2CSCR_CNTR.B.IFLG);
}


IFX_INLINE void IfxScrI2c_transmitStop(void)
{
    SCR_I2CSCR_CNTR.B.STP = 1u;
}


IFX_INLINE void IfxScrI2c_transmitStart(void)
{
    SCR_I2CSCR_CNTR.B.STA = 1u;
}
