...
void IfxGeth_Eth_initBridge(IfxGeth_Eth *geth, IfxGeth_Eth_BridgeConfig *config)
{
    ...
    if (gethSFR->BRIDGE.FORWARD_CONTROL.B.Q_CH_MAPPING_EN) /* As per HWA, following registers are configured only for two port mode */
    {
        ...
        gethSFR->BRIDGE.FORWARD_CONTROL.B.PBL = config->forwardPathTxPBL;
    }
}
...
void IfxGeth_Eth_initModule(IfxGeth_Eth *geth, IfxGeth_Eth_Config *config)
{
    ...
    /* Configure DMA */
    IfxGeth_Eth_configureDMA(geth, &config->dma);
    IfxGeth_PortIndex portIndex = IfxGeth_PortIndex_1;
...
void IfxGeth_Eth_initModuleConfig(IfxGeth_Eth_Config *config, Ifx_GETH *gethSFR)
{
    const IfxGeth_Eth_Config defaultConfig = {
        ...
        .dma                                                  = {
            ...
            .userDefinedBurstLength      = 0,
            .undefinedBurstLength        = FALSE,
...
void IfxGeth_Eth_writeHeader(uint8 *txBuffer, uint8 *destinationAddress, uint8 *sourceAddress, uint32 payloadLength)
{
    ...
    /* packet size */   
    *txBuffer++ = (uint8)(payloadLength >> 8);      /* Equivalent to *txBuffer++ = (uint8)(payloadLength / 256); */
    *txBuffer   = (uint8)(payloadLength & 0xFF);    /* Equivalent to *txBuffer   = (uint8)(payloadLength % 256);*/
}
...
void IfxGeth_Eth_configureDMA(IfxGeth_Eth *geth, IfxGeth_Eth_DmaConfig *dmaConfig)
{
    ...
    if (dmaConfig->userDefinedBurstLength != 0)
    {
        gethSFR->DMA.SYSBUS_MODE.U |= dmaConfig->userDefinedBurstLength << 1;
    }
    ...
    for (channelIndex = 0; channelIndex < IFXGETH_NUM_DMA_CHANNELS; channelIndex++)
    {
        ...
        if (dmaConfig->txChannel[channelIndex].channelEnable)
        {
            ...
            if ((dmaConfig->txInterrupt[channelIndex].priority > 0) || (dmaConfig->txInterrupt[channelIndex].provider == IfxSrc_Tos_dma0))
#endif
            {
        ...
        if (dmaConfig->rxChannel[channelIndex].channelEnable)
        {
            ...
            if ((dmaConfig->rxInterrupt[channelIndex].priority > 0) || (dmaConfig->rxInterrupt[channelIndex].provider == IfxSrc_Tos_dma0))
#endif
            {
...
void IfxGeth_Eth_initReceiveDescriptors(IfxGeth_Eth *geth, IfxGeth_Eth_RxChannelConfig *config)
{
    ...
    IFX_ASSERT(IFX_VERBOSE_LEVEL_WARNING, ((config->rxBuffer1Size & 0x7) == 0)); /* Equivalent to IFX_ASSERT(IFX_VERBOSE_LEVEL_WARNING, (config->rxBuffer1Size) % 8 == 0); */
    ...
    for (i = 0; i < IFXGETH_MAX_RX_DESCRIPTORS - 1; i++)
    {
    ...
    /* Descriptor pointed by tail pointer is not owned by the DMA */
    descr->RDES3.U = 0;
    ...
    /* set the buffer size */
    geth->gethSFR->DMA.CH[channelId].RX_CONTROL.B.RBSZ      = config->rxBuffer1Size / 8;
...
void IfxGeth_Eth_initTransmitDescriptors(IfxGeth_Eth *geth, IfxGeth_Eth_TxChannelConfig *config)
{
    ...
    geth->gethSFR->DMA.CH[channelId].TXDESC_TAIL_LPOINTER.U = (uint32)IfxGeth_Eth_getBaseTxDescriptor(geth, channelId);
...
{
    Ifx_GETH *const        gethSFR   = geth->gethSFR;
    IfxGeth_RxDescr *const baseDescr = (IfxGeth_RxDescr *const)gethSFR->DMA.CH[channelId].RXDESC_LIST_LADDRESS.U;
    const int              num_descs = gethSFR->DMA.CH[channelId].RX_CONTROL2.B.RDRL + 1;
    IfxGeth_RxDescr *const lastDescr = &baseDescr[num_descs];
    ...
    int tail_pointer_index  = tail_pointer - baseDescr;

    tail_pointer->RDES0.U = (geth->rxChannel[channelId].bufferSize * tail_pointer_index) + geth->rxChannel[channelId].buffer1Address;
    tail_pointer->RDES3.U = ((1U << 30) | (1U << 31));  /* IOC (bit 30) + OWN (bit 31) */
