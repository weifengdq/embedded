...
#endif /* (IFX_PROT_ENABLED == 1U) */
#endif /* #if IFXGETH_IS_PROTE_AVAILABLE */
    gethSFR->CLC.B.DISR = 1;        /* set the disable request */
#if IFXGETH_IS_PROTE_AVAILABLE
#if (IFX_PROT_ENABLED == 1U)
    IfxApProt_setState((Ifx_PROT_PROT *)&(gethSFR->PROTE), IfxApProt_State_run);
...
#if IFXGETH_IS_PROTE_AVAILABLE
	IfxApProt_init((Ifx_PROT_PROT *)&gethSFR->PROTE, &config->proteConfig);
#endif /* #if IFXGETH_IS_PROTE_AVAILABLE */
...
#if (IFXGETH_NUM_PORTS_IN_DEVICE > 1)
    for (index = 0; index < IFXGETH_NUM_PORTS; index++)
    {
        IfxApApu_init((Ifx_ACCEN_ACCEN *)&MODULE_GETH0.ACCENMAC[index], apConfig);
    }
#elif (IFXGETH_NUM_PORTS_IN_DEVICE == 1)
    for (index = 1; index < IFXGETH_NUM_PORTS; index++)
    {
        IfxApApu_init((Ifx_ACCEN_ACCEN *)&MODULE_GETH0.ACCENMAC[index], apConfig);
    }
#endif /* #if (IFXGETH_NUM_PORTS_IN_DEVICE > 1) */
...
void IfxGeth_Bridge_getInterruptStatus(Ifx_GETH *gethSFR, IfxGeth_Bridge_InterruptStatusFlags *flags)
{
    ...
    flags->ethernetBridge = gethSFR->BRIDGE.INTERRUPT_STATUS.B.ETHBRIS;
...
#endif /* IFXGETH_IS_BRIDGE_AVAILABLE */
