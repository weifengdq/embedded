...
void IfxLeth_configureAccessToLeth(Ifx_LETH *lethSFR, IfxApApu_ApuConfig *apConfig)
{
    uint8 index;

    IfxApApu_init((Ifx_ACCEN_ACCEN *)&(lethSFR->ACCENGLOBAL), apConfig);

    for (index = 0; index < IFXLETH_NUM_PORTS; index++)
    {
        IfxApApu_init((Ifx_ACCEN_ACCEN *)&(lethSFR->ACCENMAC[index]), apConfig);
    }

    for (index = 0; index < IFXLETH_NUM_DMA_CHANNELS; index++)
    {
        IfxApApu_init((Ifx_ACCEN_ACCEN *)&(lethSFR->ACCENCH[index]), apConfig);
    }
}
...
void IfxLeth_setupRmiiOutputPins(Ifx_LETH *lethSFR, IfxLeth_PortIndex portIndex, const IfxLeth_RmiiPins *rmiiPins)
#else
...
#endif /* #if IFXLETH_USE_PORTINDEX */ 
{
...

    {
        if (rmiiPins->txEn != NULL_PTR)
        {
            ...
            if (rmiiPins->txEn->disablePcsr == FALSE)
            {
        ...
        if (rmiiPins->txd0 != NULL_PTR)
        {
            ...
            if (rmiiPins->txd0->disablePcsr == FALSE)
            {
        ...
        if (rmiiPins->txd1 != NULL_PTR)
        {
            ...
            if (rmiiPins->txd1->disablePcsr == FALSE)
            {
...
#if IFXLETH_USE_PORTINDEX
        if ((rmiiPins->txd0->pin.pinIndex == 10) && (rmiiPins->txd0->pin.port == (Ifx_P *)&MODULE_P20))
        {
            lethSFR->P[portIndex].PORTCTRL1.B.RMII_P20_TXD0 = 1;
        }

        if ((rmiiPins->txd1->pin.pinIndex == 7) && (rmiiPins->txd1->pin.port == (Ifx_P *)&MODULE_P20))
        {
            lethSFR->P[portIndex].PORTCTRL1.B.RMII_P20_TXD1 = 1;
        }

        if ((rmiiPins->txd1->pin.pinIndex == 9) && (rmiiPins->txd1->pin.port == (Ifx_P *)&MODULE_P20))
        {
            lethSFR->P[portIndex].PORTCTRL1.B.RMII_P20_TXD1 = 2;
        }
#endif	/* #if IFXLETH_USE_PORTINDEX */	
    }
}
...
void IfxLeth_setupRmiiInputPins(Ifx_LETH *lethSFR, IfxLeth_PortIndex portIndex, const IfxLeth_RmiiPins *rmiiPins)
{
...

    {
        ...
        if (rmiiPins->refClk != NULL_PTR)
        {
            ...
            if (rmiiPins->refClk->disablePcsr == FALSE)
            {
        ...
        if (rmiiPins->rxd0 != NULL_PTR)
        {
            ...
            if (rmiiPins->rxd0->disablePcsr == FALSE)
            {
        ...
        if (rmiiPins->rxd1 != NULL_PTR)
        {
            ...
            if (rmiiPins->rxd1->disablePcsr == FALSE)
            {
...
#if IFXLETH_USE_PORTINDEX	
    IfxLeth_setupRmiiOutputPins(lethSFR, portIndex, rmiiPins);
#else
...
void IfxLeth_setPhyInterface(Ifx_LETH *lethSFR, IfxLeth_PortIndex portIndex, IfxLeth_PhyInterface phyInterface, IfxLeth_PhyInterfacePhase initPhase)
{
    ...
    case IfxLeth_PhyInterface_sgmii_1000:
    {
        if (initPhase == IfxLeth_PhyInterfacePhase_1)
        {
            lethSFR->P[portIndex].PORTCTRL0.B.EPR = 4;
        }
        else
        {
            lethSFR->PORT[portIndex].CORE.MAC_CONFIGURATION.B.FES = 0;
...
void IfxLeth_enableRxdControl(Ifx_LETH *lethSFR, IfxLeth_PortIndex portIndex, boolean enable)
{
    lethSFR->P[portIndex].PORTCTRL0.B.RXDEN = enable;
}
