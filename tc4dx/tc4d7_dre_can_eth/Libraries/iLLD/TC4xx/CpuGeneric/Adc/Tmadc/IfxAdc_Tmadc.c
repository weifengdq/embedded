...
void IfxAdc_Tmadc_configureEmux(Ifx_ADC_TMADC *tmadc, IfxAdc_Tmadc_EmuxConfig *emuxConfig)
{
    ...
    emuxCfg.B.CISEL2 = emuxConfig->emuxCi2;
    emuxCfg.B.CISEL3 = emuxConfig->emuxCi3;
...
void IfxAdc_Tmadc_initGroup(IfxAdc_Tmadc_Group *grp, IfxAdc_Tmadc_GroupConfig *config)
{
    ...
    {
        /* When timestamp is disabled, use AW0 result registers.
         * AW0.RES[0..15] is a flat, contiguous array with no reserved gaps,
         * making it safe for DMA across all 16 result registers.
         */
        grp->sourceAddress = (uint32 *)(&(modptr->AW0.RES[config->baseResultReg].U));
    }
...
IFX_STATIC void IfxAdc_Tmadc_configureDmaForGroup(IfxAdc_Tmadc_GroupConfig *groupConfig, IfxAdc_Tmadc_Group *group)
{
    ...
    {
        /* NOTE: When timestamp is enabled, the DMA source is the AW2 address window.
         * AW2 has a 32-byte reserved region between channels 6 and 7 (offsets 0x20-0x3F).
         * The DMA performs a single linear read and cannot skip this reserved region.
         * If numChannels exceeds 7, the DMA will access the reserved region and
         * trigger a bus error trap. Ensure numChannels <= 7 and all channels
         * fall within the same AW2 block (0-6, 7-13, or 14-15). */
