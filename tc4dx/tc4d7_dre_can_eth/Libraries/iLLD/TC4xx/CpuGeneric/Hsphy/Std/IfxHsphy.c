...
IFX_CONST uint32 IfxHsphy_Sgbt_paramConfig[3][IfxHsphy_SgbtRefClk_max][IfxHsphy_SgbtParamIndex_maxParamIndex] = {
    {
        {125, 2, 0, 6,  0, 12, 0,   1000, 8,  1, 1, 1, 15, 0},         /* Parameter values for 20MHz SGBT 2.5Gbps mode */
        {50,  3, 0, 12, 2, 10, 0,   1000, 10, 1, 1, 1, 15, 0},         /* Parameter values for 25MHz SGBT 2.5Gbps mode */
        {50,  3, 0, 6,  0, 12, 0,   1000, 20, 2, 1, 1, 15, 0}          /* Parameter values for 100MHz SGBT 2.5Gbps mode */
    },                                                                 /* values for SGBT 2.5Gbps mode */
    {
        {125, 0, 0, 6,  0, 12, 0,   1000, 8,  1, 0, 1, 15, 0},         /* Parameter values for 20MHz SGBT 5Gbps mode */
        {50,  1, 0, 12, 2, 10, 0,   1000, 10, 1, 0, 1, 15, 0},         /* Parameter values for 25MHz SGBT 5Gbps mode */
        {100, 0, 0, 4,  0, 12, 0,   1000, 20, 2, 0, 1, 15, 0}          /* Parameter values for 100MHz SGBT 5Gbps mode */
    },                                                                 /* values for SGBT 5Gbps mode. */                                                                   /* values for SGMII 5G mode. */
    {
        {78,  1, 0, 5,  0, 10, 260, 1094, 7,  1, 0, 1, 15, 0},         /* Parameter values for 20MHz SGBT 6.25Gbps mode */
        {125, 0, 0, 12, 2, 10, 0,   1000, 8,  1, 0, 1, 15, 0},         /* Parameter values for 25MHz SGBT 6.25Gbps mode */
        {125, 0, 0, 0,  0, 12, 0,   1000, 16, 1, 0, 1, 15, 0}          /* Parameter values for 100MHz SGBT 6.25Gbps mode */
    }
};

IFX_CONST uint32 IfxHsphy_Sgbt_refClkConfig[IfxHsphy_SgbtRefClk_max][IfxHsphy_SgbtRefClkBits_max] = {
    {0, 0, 0}, /* parameter values for 20Mhz/25Mhz frequency selection */
...
boolean IfxHsphy_resetModule(Ifx_HSPHY *hsphyRegPtr)
{
    ...
    while (1 == hsphyRegPtr->RST.STAT.B.KRST)    /* Wait until reset is released */
    {
        IFXHSPHY_LOOP_TIMEOUT_CHECK(timeoutCycleCount, timeOutError);
    }
...
#else
boolean IfxHsphy_resetXpcs(Ifx_HSPHY *hsphyRegPtr, const IfxHsphy_XpcsParams *xpcs)
#endif /* #if IFXHSPHY_IS_GETH_AVAILABLE */
{
    uint32 timeoutCycleCount = IFXHSPHY_MAX_TIMEOUT;
    uint8  timeOutError      = 0U;
...
#else
    while (hsphyRegPtr->XPCS[xpcs->xpcsIndex].PCS.SR_XS_CTRL1.B.RST == 0)
#endif /* #if IFXHSPHY_IS_GETH_AVAILABLE */		
    {
        IFXHSPHY_LOOP_TIMEOUT_CHECK(timeoutCycleCount, timeOutError);
    }
...
#endif /* IFXHSPHY_IS_PADINTERFACE_USXSGMII_AVAILABLE */


boolean IfxHsphy_Geth_rgmiiInit(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_Geth_RgmiiCfg *config)
{
    /*b Module is enabled in the IfxHsphy_enableModule() */
    uint32 timeoutCycleCount = IFXHSPHY_MAX_TIMEOUT;
    uint8  timeOutError      = 0U;
    hsphyRegPtr->CMNCFG.B.FSR     = 1;                      
    /* To select 1:RGMII pads for XGMAC1,0:RGMII pads for XSPI   */    
    ...
    /* Polling to check the lock status */
...
void IfxHsphy_Xspi_deInit(Ifx_HSPHY *hsphyRegPtr)
{
...
boolean IfxHsphy_Xspi_xspiInit(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_Xspi_XspiCfg *config)
{
    ...
    hsphyRegPtr->DLL.CFG.B.RXSAMPLESEL = config->dllCfg.rxSampleSel;
    hsphyRegPtr->DLL.CFG.B.PEN3VX      = config->dllCfg.dllOutputVolt;
...
IfxHsphy_SgmiiSpeedConfigStatus IfxHsphy_setSgmiiSpeedMode(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_TrgtDeviceSpeed trgtSpeed, const IfxHsphy_XpcsParams *xpcs)
{
    IfxHsphy_SgmiiSpeed sgmiiSpeed        = IfxHsphy_SgmiiSpeed_1G;
    boolean             relvalCheck;
...
#if IFXHSPHY_IS_PADINTERFACE_SGBT_AVAILABLE
IfxHsphy_TpcsSpeedConfigStatus IfxHsphy_setTpcsSpeedMode(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_TrgtDeviceSpeed trgtSpeed, const IfxHsphy_TpcsParams *tpcs)
{
    uint32                   timeoutCycleCount = IFXHSPHY_MAX_TIMEOUT;
    uint8                    timeOutError      = 0U;
    boolean                  relvalCheck;
    IfxHsphy_TrgtDeviceSpeed sgbtSpeed         = trgtSpeed;

    if ((sgbtSpeed != IfxHsphy_TrgtDeviceSpeed_Sgbt_2P5G) && (sgbtSpeed != IfxHsphy_TrgtDeviceSpeed_Sgbt_5G) && (sgbtSpeed != IfxHsphy_TrgtDeviceSpeed_Sgbt_6P25G))
    {
        return IfxHsphy_TpcsSpeedConfigStatus_invalidConfigError;
    }

    relvalCheck = IfxHsphy_tpcsMpllDisable(hsphyRegPtr, tpcs->tpcsIndex);

    if (relvalCheck == TRUE)
    {
        return IfxHsphy_TpcsSpeedConfigStatus_timeOutError;
    }

    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_25G_REF_CLK_CTRL.B.REF_CLK_DIV2   = IfxHsphy_Sgbt_refClkConfig[tpcs->sgbtRefClk][IfxHsphy_SgbtRefClkBits_refMpllaDiv2];
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_25G_REF_CLK_CTRL.B.REF_MPLLA_DIV2 = IfxHsphy_Sgbt_refClkConfig[tpcs->sgbtRefClk][IfxHsphy_SgbtRefClkBits_refClKDiv2];
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_25G_REF_CLK_CTRL.B.REF_RANGE      = IfxHsphy_Sgbt_refClkConfig[tpcs->sgbtRefClk][IfxHsphy_SgbtRefClkBits_refRange];

    hsphyRegPtr->TPCS.MII.VR.DIG_CTRL1.B.VR_RST                           = 1;

    while (1 == hsphyRegPtr->TPCS.MII.VR.DIG_CTRL1.B.VR_RST)
    {
        IFXHSPHY_LOOP_TIMEOUT_CHECK(timeoutCycleCount, timeOutError);
    }
    ...
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_MPLLA_CTRL0.B.MPLLA_MULTIPLIER     = IfxHsphy_Sgbt_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_mpllaMultiplier];
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_MPLLA_CTRL2.B.MPLLA_TX_CLK_DIV     = IfxHsphy_Sgbt_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_mpllaTxClkDiv];
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_MPLLA_CTRL2.B.MPLLA_DIV16P5_CLK_EN = IfxHsphy_Sgbt_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_mpllaDiv16P5ClkEnable];
    hsphyRegPtr->TPCS.MII.VR.MP_8G_MPLLA_CTRL6.B.CP_PROP                   = IfxHsphy_Sgbt_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_cpProp];
    hsphyRegPtr->TPCS.MII.VR.MP_8G_MPLLA_CTRL6.B.CP_INT                    = IfxHsphy_Sgbt_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_cpInt];
    hsphyRegPtr->TPCS.MII.VR.MP_8G_MPLLA_CTRL7.B.CP_PROP_GS                = IfxHsphy_Sgbt_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_cpPropGs];
    hsphyRegPtr->TPCS.MII.VR.MP_16G_MPLLA_CTRL1.B.MPLLA_FRACN_CTRL         = IfxHsphy_Sgbt_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_mpllaFrancCtrl];
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_25G_VCO_CAL_LD0.B.VCO_LD_VAL_0     = IfxHsphy_Sgmii_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_vcoLdVal_0];
    hsphyRegPtr->TPCS.MII.VR.MP_12G_VCO_CAL_REF0.B.VCO_REF_LD_0            = IfxHsphy_Sgmii_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_vcoRefLd_0];
    hsphyRegPtr->TPCS.MII.VR.MP_16G_25G_MISC_CTRL2.B.SUP_MISC              = IfxHsphy_Sgmii_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_supMisc];
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_25G_TX_RATE_CTRL.B.TX0_RATE        = IfxHsphy_Sgmii_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_tx0Rate];
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_TX_GENCTRL2.B.TX0_WIDTH            = IfxHsphy_Sgmii_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_tx0Width];
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_25G_TX_BOOST_CTRL.B.TX0_IBOOST     = IfxHsphy_Sgmii_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_tx0Iboost];
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_25G_TX_GENCTRL1.B.VBOOST_EN_0      = IfxHsphy_Sgmii_paramConfig[sgbtSpeed][tpcs->sgbtRefClk][IfxHsphy_SgbtParamIndex_vBoostEn_0];

    relvalCheck                                                            = IfxHsphy_sgbtMpllEnable(hsphyRegPtr, tpcs->tpcsIndex);
    ...
    return IfxHsphy_TpcsSpeedConfigStatus_success;
}


boolean IfxHsphy_tpcsMpllDisable(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_TpcsIndex tpcsIndex)
{
    uint32 timeoutCycleCount = IFXHSPHY_MAX_TIMEOUT;
    uint8  timeOutError      = 0U;
    /* Disable MPLL */
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_25G_MPLL_CMN_CTRL.B.MPLL_EN_0 = 0;
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_TX_GENCTRL2.B.TX_REQ_0        = 1;

    while (1 == hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_TX_GENCTRL2.B.TX_REQ_0)
    {
        IFXHSPHY_LOOP_TIMEOUT_CHECK(timeoutCycleCount, timeOutError);
    }
    ...
    while (1 == hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_25G_TX_STS.B.TX_ACK_0)
    {
        IFXHSPHY_LOOP_TIMEOUT_CHECK(timeoutCycleCount, timeOutError);
    }
...
boolean IfxHsphy_sgbtMpllEnable(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_TpcsIndex tpcsIndex)
{
    uint32 timeoutCycleCount = IFXHSPHY_MAX_TIMEOUT;
    uint8  timeOutError      = 0U;
    /* Enable/Bring-up MPLL in 1.25Gb/s rate */
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_25G_MPLL_CMN_CTRL.B.MPLL_EN_0 = 1;
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_TX_GENCTRL2.B.TX_REQ_0        = 1;
    ...
    hsphyRegPtr->TPCS.MII.VR.DIG_CTRL1.B.INIT = 1;

    while (1 == hsphyRegPtr->TPCS.MII.VR.DIG_CTRL1.B.INIT)
    {
        IFXHSPHY_LOOP_TIMEOUT_CHECK(timeoutCycleCount, timeOutError);
    }
    ...
    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_25G_TX_GENCTRL1.B.TX_CLK_RDY_0 = 0;

    hsphyRegPtr->TPCS.MII.SR.CTRL.B.LPM                                = 1;

    timeoutCycleCount                                                  = IFXHSPHY_MAX_TIMEOUT;

    while (6 == hsphyRegPtr->TPCS.MII.VR.DIG_STS.B.PSEQ_STATE)
    {
        IFXHSPHY_LOOP_TIMEOUT_CHECK(timeoutCycleCount, timeOutError);
    }
    ...
    /* Power up the DWC_xpcs */
    hsphyRegPtr->TPCS.MII.SR.CTRL.U &= (uint32)(~((1 << IFX_HSPHY_TPCS_MII_SR_CTRL_LPM_OFF) | (1 << IFX_HSPHY_TPCS_MII_SR_CTRL_RST_OFF)));      

    timeoutCycleCount                = IFXHSPHY_MAX_TIMEOUT;

    while (hsphyRegPtr->TPCS.MII.VR.DIG_STS.B.PSEQ_STATE != IfxHsphy_XpcsPowerUpSeqState_powerGood)
    {
        IFXHSPHY_LOOP_TIMEOUT_CHECK(timeoutCycleCount, timeOutError);
    }

    hsphyRegPtr->TPCS.MII.VR.MP_12G_16G_25G_TX_GENCTRL1.B.TX_CLK_RDY_0 = 1;
    return (boolean)timeOutError;
}
#endif /* #if IFXHSPHY_IS_PADINTERFACE_SGBT_AVAILABLE */
