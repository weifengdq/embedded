...
uint32 IfxEgtm_Atom_Agc_buildFeature(uint16 enableMask, uint16 disableMask, uint8 bitfieldOffset)
{
    uint8  i;
    uint32 reg  = 0u;
    ...
    for (i = 0u; i < IFXEGTM_ATOM_NUM_AGC_CHANNELS; i++)
    {
        ...
        if ((mask & 0x1u) == 0x1u)
        {
            reg |= (uint32)IfxEgtm_FeatureControl_enable << shift;
        }

        if ((mask & 0x10000u) == 0x10000u)
        {
            reg |= (uint32)IfxEgtm_FeatureControl_disable << shift;
        }
...
uint32 IfxEgtm_Atom_Agc_buildFeatureForChannel(IfxEgtm_Atom_Ch channel, boolean enabled, uint8 bitfieldOffset)
{
    uint32 reg = 0u;
...
float32 IfxEgtm_Atom_Ch_getFrequency(Ifx_EGTM_CLS_ATOM *atom, IfxEgtm_Atom_Ch channel)
{
    float32                    frequency         = 0.0f, sourceFrequency;
    uint32                     periodTicks       = 0u;
    IfxEgtm_Atom_Ch_ResetEvent counterResetEvent = (IfxEgtm_Atom_Ch_ResetEvent)atom->CH[channel].CTRL.B.RST_CCU0;

    while ((counterResetEvent != IfxEgtm_Atom_Ch_ResetEvent_onCm0) &&
           (channel > IfxEgtm_Atom_Ch_0))       /* Counter is reset from previous channel */
    {
        channel           = (IfxEgtm_Atom_Ch)((uint8)channel - 1u);
...
float32 IfxEgtm_Atom_Ch_getDuty(Ifx_EGTM_CLS_ATOM *atom, IfxEgtm_Atom_Ch channel)
{
    float32                    duty = 0.0f;
    ...
    if (counterResetEvent == IfxEgtm_Atom_Ch_ResetEvent_onCm0)  /* Period is determined by own channel */
    {
        periodReg = cm0Reg;
    }
    else    /* Period is determined by some previous channel */
    {
        ...
        while ((tempChannel > 0) &&
               (atom->CH[tempChannel].CTRL.B.RST_CCU0 != (uint8)IfxEgtm_Atom_Ch_ResetEvent_onCm0))
        {
            tempChannel--;
        }
