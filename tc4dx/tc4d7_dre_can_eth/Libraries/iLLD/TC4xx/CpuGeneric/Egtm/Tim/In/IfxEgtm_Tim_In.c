...
boolean IfxEgtm_Tim_In_init(IfxEgtm_Tim_In *driver, const IfxEgtm_Tim_In_Config *config)
{
    ...
    case IfxEgtm_Tim_In_Src_channelCurrentOrPrevious:
        ...
        if (srcForTimIn == IfxEgtm_Tim_In_SrcForTimIn_currentChannel)
        {
    ...
    case IfxEgtm_Tim_In_Src_auxiliary:
        driver->tim->IN_SRC.U = (((uint32)1u << IFX_EGTM_CLS_TIM_IN_SRC_MODE0_OFF) + ((uint32)2u << IFX_EGTM_CLS_TIM_IN_SRC_VAL0_OFF)) << ((uint8)channelIndex * (IFX_EGTM_CLS_TIM_IN_SRC_VAL0_LEN + IFX_EGTM_CLS_TIM_IN_SRC_MODE0_LEN));
        break;

    case IfxEgtm_Tim_In_Src_software:
        driver->tim->IN_SRC.U = (((uint32)2u << IFX_EGTM_CLS_TIM_IN_SRC_MODE0_OFF) + ((uint32)1u << IFX_EGTM_CLS_TIM_IN_SRC_VAL0_OFF)) << ((uint8)channelIndex * (IFX_EGTM_CLS_TIM_IN_SRC_VAL0_LEN + IFX_EGTM_CLS_TIM_IN_SRC_MODE0_LEN));
...
void IfxEgtm_Tim_In_initConfig(IfxEgtm_Tim_In_Config *config, Ifx_EGTM *egtm)
{
    ...
    config->srcForTimIn                  = IfxEgtm_Tim_In_SrcForTimIn_currentChannel;
    config->useLUT                       = FALSE;
