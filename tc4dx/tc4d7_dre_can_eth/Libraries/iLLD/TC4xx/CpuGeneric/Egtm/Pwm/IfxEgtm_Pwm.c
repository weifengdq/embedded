...
#endif /* #if IFXEGTM_PWM_OUTPUT_STATUS */
 ...
 *
 * \param[in] clusterSFR Cluster SFR
 * \param[in] subModule  GTM Sub Module
 *                       Range: \ref: IfxEgtm_Pwm_SubModule
 * \param[in] channel    PWM channel
 *                       Range: \ref: IfxEgtm_Pwm_SubModule_Ch
 *
 * \retval Returns pointer to DTM if available for given configuration
 */
 ...
 *
 * \param[inout] glbCtrl  Global control register
 * \param[in]    upenMask UPEN Mask of channel
 ...
 *
 * \param[in]    clusterSFR  Pointer to the Cluster object
 * \param[inout] subModule   ATOM/TOM
 *                           Range: \ref: IfxEgtm_Pwm_SubModule
 * \param[in]    channel     Channel index
 *                           Range: \ref: IfxEgtm_Pwm_SubModule_Ch
 * \param[in]    clockSource Configured clock source
 *                           Range: \ref: IfxEgtm_Pwm_ClockSource
 *
 * \retval None
 */
 ...
 * \param[in]    channel    Channel index
 *                          Range: \ref: IfxEgtm_Pwm_SubModule_Ch
 * \param[in]    event      Reset event
 *                          Range: \ref: IfxEgtm_Pwm_ResetEvent
 *
 * \retval None
 */
 ...
 * \param[in]    channel    Channel index
 *                          Range: \ref: IfxEgtm_Pwm_SubModule_Ch
 * \param[in]    alignment  PWM signal alignment
 *                          Range: \ref: IfxEgtm_Pwm_Alignment
 *
 * \retval None
 */
 ...
 * \param[in]    channel    Channel index
 *                          Range: \ref: IfxEgtm_Pwm_SubModule_Ch
 * \param[in]    enabled    TRUE: Generate trigger
 ...
 *
 * \param[in] pwm PWM handle
 * \param[in] pin Pin configuration
 *
 * \retval None
 */
 ...
 * \param[in]    channel      Channel index
 *                            Range: \ref: IfxEgtm_Pwm_SubModule_Ch
 * \param[in]    compareValue Generate trigger when counter reaches this value
 ...
 *
 * \param[in] pwm         PWM handle
 * \param[in] egtmSFR     egtmSFR
 * \param[in] clockSource Clock configuration
 *
 ...
 * \param[in]    configIndex Index of channel to be initialized
 ...
 *
 * \param[inout] pwm       PWM handle
 * \param[in]    interrupt Interrupt configuration
...
IFX_INLINE void IfxEgtm_Pwm_disableChannelsSyncUpdate(IfxEgtm_Pwm *pwm)
{
    ...
    /* Disables UPEN in TGC0/1 or AGC if all channels are in one control unit */
    if (pwm->globalControl.reg1 == NULL_PTR)
    {
        *globalctrl->reg0 = pwm->globalControl.upenMask0 & (uint32)IFXEGTM_PWM_UPEN_DISABLE_VALUE;
    }
    /* Disables UPEN in TGC0,1 if all channels not in one control unit */
    else
    {
        *globalctrl->reg0 = pwm->globalControl.upenMask0 & (uint32)IFXEGTM_PWM_UPEN_DISABLE_VALUE;
...
IFX_INLINE void IfxEgtm_Pwm_configureTimeBaseTrigger(Ifx_EGTM_CLS *clusterSFR, IfxEgtm_Pwm_SubModule subModule, IfxEgtm_Pwm_SubModule_Ch channel, uint32 compareValue)
{
    ...
    actTbReg.B.ACT_TB  = compareValue;      /* Compares Value */
    actTbReg.B.TBU_SEL = 0x0u;              /* TBU_TS0 */
    actTbReg.B.TB_TRIG = 0x1u;              /* Triggers when compare value matches */
...
IFX_INLINE void IfxEgtm_Pwm_updateBaseChannelFrequency(IfxEgtm_Pwm *pwm, float32 requestFrequency)
{
    ...
    /* Calculates period ticks (round to nearest integer) */
...
void IfxEgtm_Pwm_init(IfxEgtm_Pwm *pwm, IfxEgtm_Pwm_Channel *channels, IfxEgtm_Pwm_Config *config)
{
    ...
    /* 3. Gets Clock information */
    pwm->sourceFrequency = IfxEgtm_Pwm_getSubModuleClockFrequency(pwm, config->egtmSFR, config->clockSource);
#if IFXEGTM_PWM_IS_HIGH_RES_AVAILABLE	
    pwm->sourceFrequency = (config->highResEnable == TRUE) ? (pwm->sourceFrequency * 32.0f) : pwm->sourceFrequency;
#endif /* #if IFXEGTM_PWM_IS_HIGH_RES_AVAILABLE */

    pwm->dtmFrequency    = IfxEgtm_Pwm_getDtmClockFrequency(pwm, config->egtmSFR, pwm->dtmClockSource);
#if IFXEGTM_PWM_IS_HIGH_RES_AVAILABLE
    pwm->dtmFrequency    = (config->dtmHighResEnable == TRUE) ? (pwm->dtmFrequency * 32.0f) : pwm->dtmFrequency;
#endif /* #if IFXEGTM_PWM_IS_HIGH_RES_AVAILABLE */

#if IFXEGTM_PWM_SW_RESET
    /* 4. Performs software reset of PWM channels */
    idx = (uint8)config->channels[0].timerCh;

    while (idx <= (uint8)config->channels[config->numChannels - 1u].timerCh)   /* Resets all channels in range even if not configured by user */
    {
        if (pwm->subModule == IfxEgtm_Pwm_SubModule_atom)
        {
...
#endif /* #if IFXEGTM_PWM_SW_RESET */

    /* 5. Initializes PWM channels */
    ...
    /* 6. Updates masks and pointers to GLB_CTRL registers */
...
IFX_STATIC void IfxEgtm_Pwm_initChannel(IfxEgtm_Pwm *pwm, IfxEgtm_Pwm_Config *config, IfxEgtm_Pwm_SyncChannelIndex configIndex)
{
    ...
    /* 0. Stores pointers to registers for faster access later */
    {
    ...
    /* 1. Configures global control unit */
    {
        /* 1.1 Enables update of CM0/1 from shadow registers */
    ...
    Ifx_ActiveState channelPolarity = (channelConfig->output != NULL_PTR) ? channelConfig->output->polarity : Ifx_ActiveState_high;
    {
        ...
        if (subModule == IfxEgtm_Pwm_SubModule_atom)
        {
            /* 2.3 Sets ATOM signal output mode as PWM */
            clusterSFR->ATOM.CH[channelIndex].CTRL.B.MODE = (uint8)IfxEgtm_Atom_Mode_outputPwm;

#if IFXEGTM_PWM_IS_HIGH_RES_AVAILABLE
            /* 2.4 Setup HRPWM */
            if ((config->highResEnable == TRUE) &&
                (channelConfig->output != NULL_PTR))
            {
                IfxEgtm_Hrpwm_Ch highResCh = channelConfig->output->pin->hrpwm.channel;

                /* Enables HRES in ATOM Channel */
                clusterSFR->ATOM.CH[channelIndex].CTRL2.B.HRES = 1u;

                /* Enables HRPWM module */
                IfxEgtm_setHrpwmEnable(pwm->egtmSFR, (IfxEgtm_Hrpwm)pwm->cluster, TRUE);

                /* Enables HRPWM channel */
                IfxEgtm_setHrpwmChannelEnable(pwm->egtmSFR, (IfxEgtm_Hrpwm)pwm->cluster, highResCh, TRUE);

                /* Connects Atom channel x to HRPWM channel x */
                IfxEgtm_Atom_connectHrpwmChannel(pwm->cluster, (IfxEgtm_Atom_Ch)channelIndex, highResCh);

                if (channelConfig->output->complementaryPin != NULL_PTR)
                {
                    highResCh = channelConfig->output->complementaryPin->hrpwm.channel;

                    /* Enables HRPWM channel */
                    IfxEgtm_setHrpwmChannelEnable(pwm->egtmSFR, (IfxEgtm_Hrpwm)pwm->cluster, highResCh, TRUE);

                    /* Connects inverted DTM output x to HRPWM channel x */
                    uint32 shift = IFX_EGTM_HRPWMMUX_CHSEL_CH0_LEN * (uint32)highResCh;
                    uint32 mask  = (uint32)IFX_EGTM_HRPWMMUX_CHSEL_CH0_MSK << shift;
                    uint32 value = (((uint32)channelIndex << 1u) + 1u) << shift;

                    Ifx__ldmst(&(MODULE_EGTM.HRPWMMUX[pwm->cluster].CHSEL.U), mask, value);
                }
            }
#endif /* #if IFXEGTM_PWM_IS_HIGH_RES_AVAILABLE */			
        }
    ...
    /* 3. Performs counter related configuration */
    {
        /* 3.1 Resets counter to 0 */
        *channel->registers.CN0 = 0x0u;
        ...
        /* 3.3 Sets counter mode according to wave alignment */
        IfxEgtm_Pwm_setCounterMode(clusterSFR, subModule, channelIndex, pwm->alignment);
    }

    /* 4. Calculates and set compare value registers */
    {
        /* 4.1 Calculates and set channel PWM frequency. Frequency check already performed */
        if (baseChannel == TRUE)
        {
        ...
        /* 4.2 Calculates and set channel PWM phase shift for sync channels */
    ...
    /* 5. Configures Dead time module */
    {
        ...
        if (channelConfig->dtm != NULL_PTR)
        {
            ...
            /* 5.2 Sets DTM Clock source */
            IfxEgtm_Dtm_setClockSource(dtmSFR, pwm->dtmClockSource);

            /* 5.3 Enables dead time insertion at OUTx and OUTx_N */
            ...
            /* 5.4 Inverts the signal at OUTx_N */
            IfxEgtm_Dtm_setOutput1Select(dtmSFR, dtmChannel, IfxEgtm_Dtm_Output1Select_inverseDeadTime);

#if IFXEGTM_PWM_IS_HIGH_RES_AVAILABLE
            /* 5.5 Enables dtm high res support */
            dtmSFR->CH_DTV[dtmChannel].B.HRES = (uint8)(config->dtmHighResEnable);
#endif /* #if IFXEGTM_PWM_IS_HIGH_RES_AVAILABLE */

            /* 5.6 Configures dead time values */
            if (config->syncUpdateEnabled == TRUE)
            {
                ...
                /* Updates at rising edge if pulse is high during duty */
            ...
            /* 5.7 Sets complementary pin output polarity
            ...
            /* 5.8 Configures DTM fast shutoff */
            if (channelConfig->dtm->fastShutOff != NULL_PTR)
            {
                ...
                /* Generates constant output equal to shadow signal level once shutoff is activated */
        ...
        /* Sets complementary pin output polarity if Dead time path is not being used. */
    ...
    if (channelConfig->output != NULL_PTR)
    {
        ...
        /* Checks pin availability and configure output pin */
        if (channelConfig->output->pin != NULL_PTR)
        {
#if IFXEGTM_PWM_IS_HIGH_RES_AVAILABLE		
            if ((config->highResEnable == TRUE) || (config->dtmHighResEnable == TRUE))
            {
                IfxEgtm_PinMap_setHrpwmOut(&(pin.outputPin->hrpwm), pin.outputMode, pin.padDriver);
            }
            else
            {
        ...
        /* Checks pin availability and configure complementary output pin */
        if (channelConfig->output->complementaryPin != NULL_PTR)
        {
...
IFX_STATIC void IfxEgtm_Pwm_initInterrupts(IfxEgtm_Pwm *pwm, IfxEgtm_Pwm_InterruptConfig *interrupt, IfxEgtm_Pwm_Channel *channel)
{
    ...
    if (interrupt != NULL_PTR && interrupt->priority > 0)
    {
        boolean periodEventEnabled = interrupt->ccu0Enabled;
        boolean dutyEventEnabled   = interrupt->ccu1Enabled;

        /* Copy callback function pointers */
        channel->periodEvent = interrupt->periodEvent;
        channel->dutyEvent   = interrupt->dutyEvent;

        /* Sets notification and get src pointer */
        if (pwm->subModule == IfxEgtm_Pwm_SubModule_atom)
        {
...
IFX_STATIC void IfxEgtm_Pwm_updateGlobalControlUnitData(IfxEgtm_Pwm *pwm)
{
    ...
    /* Calculates Update enable mask for channels being used. Channels may not be consecutive. */
    ...
    if (pwm->subModule == IfxEgtm_Pwm_SubModule_atom)
    {
        /* Gets mask for channel range from first to last */
    ...
    {
        /* Enables both control units If channels are in different cluster */
...
void IfxEgtm_Pwm_startSyncedChannels(IfxEgtm_Pwm *pwm)
{
    ...
    /* 1. Checks if init is done */
    if ((pwm->state == IfxEgtm_Pwm_State_init) ||
        (pwm->state == IfxEgtm_Pwm_State_stopped))
    {
        /* 2. Uses TBU to start different TGCs */
        if (pwm->globalControl.reg1 != NULL_PTR)
        {
            /* Enables channels on trigger */
            ...
            *pwm->globalControl.endisCtrlReg1 = (~(~IFXEGTM_PWM_UPEN_ENABLE_VALUE & pwm->globalControl.upenMask1)) >> 16u;
            ...
            /* 2.3 Disables TBU_TS0 for configuration */
            tbuSFR->CHEN.B.ENDIS_CH0 = (uint8)IfxEgtm_FeatureControl_disable;

            /* 2.4 Configures AGC/TGC0/1 of pwm1 */
            ...
            /* 2.5 TBU Set up */
            /* Configures clock source and base value */
            ...
            /* 2.7 Disables TBU_TS0 */
            tbuSFR->CHEN.B.ENDIS_CH0 = (uint8)IfxEgtm_FeatureControl_disable;

            /* 2.8 Disables CM_CLK0 if enabled earlier */
            if (cmuClockEnabled == FALSE)
            {
                egtmSFR->CLS[0].CMU.CLK_EN.B.EN_CLK0 = (uint8)IfxEgtm_FeatureControl_disable;
            }
        }
        /* 2. Gives trigger to start counter of channels */
        else
        {
...
void IfxEgtm_Pwm_stopSyncedChannels(IfxEgtm_Pwm *pwm)
{
    ...
    /* 1. Checks if PWM is running */
    if (pwm->state == IfxEgtm_Pwm_State_run)
    {
        /* 2. Uses TBU to stop different TGCs */
        if (pwm->globalControl.reg1 != NULL_PTR)
        {
            /* Disables channel on trigger */
...
void IfxEgtm_Pwm_startSyncedGroups(IfxEgtm_Pwm *pwm1, IfxEgtm_Pwm *pwm2)
{
    ...
    /* 3. Disables TBU_TS0 for configuration */
    tbuSFR->CHEN.B.ENDIS_CH0 = (uint8)IfxEgtm_FeatureControl_disable;

    /* 4. Configures AGC/TGC0/1 of pwm1 */
    *pwm1->globalControl.endisCtrlReg0 = (~(~IFXEGTM_PWM_UPEN_ENABLE_VALUE & pwm1->globalControl.upenMask0)) >> 16u;
    ...
    /* 5. Configures AGC/TGC0/1 of pwm1 */
    *pwm2->globalControl.endisCtrlReg0 = (~(~IFXEGTM_PWM_UPEN_ENABLE_VALUE & pwm2->globalControl.upenMask0)) >> 16u;
    ...
    /* 6. TBU Set up */
    /* 6.1 Configures clock source and base value */
    tbuSFR->CH0_CTRL.B.LOW_RES    = 0x0u;
    tbuSFR->CH0_CTRL.B.CH_CLK_SRC = (uint8)tbuClock;
    tbuSFR->CH0_BASE.U            = 0x0u;

    /* 6.2 Starts TBU_TS0 counter */
    tbuSFR->CHEN.B.ENDIS_CH0 = (uint8)IfxEgtm_FeatureControl_enable;
    ...
    /* 9. Disables CM_CLK0 if enabled earlier */
    if (cmuClockEnabled == FALSE)
    {
        egtmSFR->CLS[0].CMU.CLK_EN.B.EN_CLK0 = (uint8)IfxEgtm_FeatureControl_disable;
    }
...
void IfxEgtm_Pwm_updateFrequency(IfxEgtm_Pwm *pwm, float32 requestFrequency)
{
    ...
    /* 2. Calculates and set base channel PWM frequency */
    IfxEgtm_Pwm_updateBaseChannelFrequency(pwm, requestFrequency);

    /* 3. Calculates and set base channel PWM duty */
...
void IfxEgtm_Pwm_updateSyncedGroupsFrequency(IfxEgtm_Pwm *pwm1, IfxEgtm_Pwm *pwm2, float32 requestFrequency)
{
    ...
    /* 2. Updates frequency of two groups */
    for (pwmIndex = 0; pwmIndex < 2; pwmIndex++)
    {
        pwm            = pwmArray[pwmIndex];
        frequencyRatio = pwm->frequency / requestFrequency;

        /* Calculates and set base channel PWM frequency */
        IfxEgtm_Pwm_updateBaseChannelFrequency(pwm, requestFrequency);

        /* Calculates and set base channel PWM duty */
        channel                 = &(pwm->channels[0]);
        newDutyTicks            = (uint32)((float32)(((float32)channel->dutyTicks * frequencyRatio) + (float32)0.5f));
        channel->dutyTicks      = newDutyTicks;
        *channel->registers.SR1 = (newDutyTicks <= pwm->periodTicks) ? newDutyTicks : newDutyTicks - pwm->periodTicks;
    ...
    /* 5. Enable UPEN */
    /* Updates first control unit to ensure counters don't get out of sync */
...
void IfxEgtm_Pwm_updateFrequencyImmediate(IfxEgtm_Pwm *pwm, float32 requestFrequency)
{
    ...
    /* 1. Calculates and set base channel PWM frequency */
    IfxEgtm_Pwm_updateBaseChannelFrequencyImmediate(pwm, requestFrequency);

    /* 2. Calculates and set base channel PWM duty */
    channel                 = &(pwm->channels[0]);
    newDutyTicks            = (uint32)((float32)(((float32)channel->dutyTicks * frequencyRatio) + (float32)0.5f));
    channel->dutyTicks      = newDutyTicks;
...
void IfxEgtm_Pwm_updateChannelPulse(IfxEgtm_Pwm *pwm, IfxEgtm_Pwm_SyncChannelIndex configIndex, float32 requestPhase, float32 requestDuty)
{
    ...
    /* 4. Enables update from shadow register */
...
{
    /* 1. Updates phase if not base channel */
    if (configIndex != IfxEgtm_Pwm_SyncChannelIndex_0)
    {
...
void IfxEgtm_Pwm_updateChannelsDuty(IfxEgtm_Pwm *pwm, float32 *requestDuty)
{
    ...
    /* 3. Enables update from shadow register */
    IfxEgtm_Pwm_enableChannelsSyncUpdate(pwm);
}
...
void IfxEgtm_Pwm_updateChannelsPulse(IfxEgtm_Pwm *pwm, float32 *requestPhase, float32 *requestDuty)
{
    ...
    /* 2. Updates duty for base channel */
...
void IfxEgtm_Pwm_updateChannelsPulseImmediate(IfxEgtm_Pwm *pwm, float32 *requestPhase, float32 *requestDuty)
{
    ...
    /* 1. Updates duty for base channel */
...
void IfxEgtm_Pwm_resetFastShutoff(IfxEgtm_Pwm *pwm)
{
    uint8 idx;

    for (idx = 0u; idx < pwm->numChannels; idx++)
    {
        IfxEgtm_Pwm_SubModule_Ch channelIndex = pwm->channels[idx].timerCh;
        Ifx_EGTM_CLS_CDTM_DTM  *dtmSFR        = IfxEgtm_Pwm_getDtmPtr(pwm->clusterSFR, pwm->subModule, channelIndex);
        IfxEgtm_Dtm_Ch          dtmChannel    = (IfxEgtm_Dtm_Ch)((uint8)((uint8)channelIndex & 0x3u));

        /* Write SHUT_OFF_RST_x bit in DTM CTRL2 to reset SHUTOFF_SYNC_x */
        IfxEgtm_Dtm_resetShutoffSignal(dtmSFR, dtmChannel);
    }
}


void IfxEgtm_Pwm_enableSelfResetOfFastShutoff(IfxEgtm_Pwm *pwm)
{
    uint8 idx;
    ...
    for (idx = 0u; idx < pwm->numChannels; idx++)
    {
        ...
        /* Sets shutoff update mode: signal PSU_SHUT_OFF=1 sets internal signal SHUTOFF_SYNC_x;
         * signal PSU_SHUT_OFF=0 clears internal signal SHUTOFF_SYNC_x */
        IfxEgtm_Dtm_setShutoffUpdateMode(dtmSFR, dtmChannel, IFXEGTM_DTM_UPD_MODE_SELF_RESET);
    }
}
...
/* Calculates output enable mask for channels being used. Channels may not be consecutive. */
    for (idx = 0; idx < pwm->numChannels; idx++)
    {
        index = pwm->channels[idx].timerCh;
