 ...
 * \retval None
 */
IFX_EXTERN void IfxEgtm_Pwm_resetFastShutoff(IfxEgtm_Pwm *pwm);
 ...
 * \retval None
 */
IFX_EXTERN void IfxEgtm_Pwm_enableSelfResetOfFastShutoff(IfxEgtm_Pwm *pwm);
...
{
    IfxEgtm_Pwm_Channel *channel     = &pwm->channels[configIndex];
    boolean              baseChannel = (configIndex == IfxEgtm_Pwm_SyncChannelIndex_0);

    /* Handle 0% duty cycle case */
    if (requestDuty <= 0.0f)
    {
        if (pwm->alignment == IfxEgtm_Pwm_Alignment_edge)
        {
            /* Workaround for edge-aligned PWM - set CM0 > MAX and CM1 = 0 */
            channel->dutyTicks = 0;

            /* For base channel, set CM0 to period+1 */
            if (baseChannel)
            {
                *channel->registers.SR0 = pwm->periodTicks + 1;
            }
            /* For sync channel, set CM0 to phase+period+1 */
            else
            {
                *channel->registers.SR0 = channel->phaseTicks + pwm->periodTicks + 1;
            }

            *channel->registers.SR1 = 0;
        }
        else
        {
            /* Center-aligned PWM - standard 0% duty cycle */
            channel->dutyTicks      = pwm->periodTicks;
            *channel->registers.SR1 = pwm->periodTicks;
        }
    ...
    else
    {
        ...
        *channel->registers.SR1 = dutyTicks;
        ...
        /* For non-zero duty, ensure CM0 is set correctly */
        if (baseChannel)
        {
            *channel->registers.SR0 = pwm->periodTicks;
        }
        else
        {
            *channel->registers.SR0 = channel->phaseTicks;
        }
    }
}
