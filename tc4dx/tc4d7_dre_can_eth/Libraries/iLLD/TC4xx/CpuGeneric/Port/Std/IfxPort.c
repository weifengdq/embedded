...
{
/* PADCFGp_SAFSEC is protected by ACCGRPy */
#if (IFX_PROT_ENABLED == 1U)
    uint8 grpIdx = port->PADCFG[pinIndex].ACCEN.B.GRP;
    IfxApProt_setState((Ifx_PROT_PROT *)&port->ACCGRP[grpIdx].PROTE, IfxApProt_State_config);
#endif
    Ifx__ldmst(&(port->PADCFG[pinIndex].SAFSEC.U), (IFX_P_PADCFG_SAFSEC_ESR_EN_MSK << IFX_P_PADCFG_SAFSEC_ESR_EN_OFF), 0U);
#if (IFX_PROT_ENABLED == 1U)
    IfxApProt_setState((Ifx_PROT_PROT *)&port->ACCGRP[grpIdx].PROTE, IfxApProt_State_run);
#endif
}
...
#endif
    /* PADCFGp_DRVCFG is protected by ACCGRPy */
#if (IFX_PROT_ENABLED == 1U)
    uint8 grpIdx = port->PADCFG[pinIndex].ACCEN.B.GRP;
    IfxApProt_setState((Ifx_PROT_PROT *)&port->ACCGRP[grpIdx].PROTE, IfxApProt_State_config);
#endif
