...
IFX_CONST IfxClock_AudioConfig      IfxClock_defaultAudioClockConfig = {
    IFX_CFG_CLOCK_XTAL_FREQUENCY,
    IFXCLOCK_CFG_APLL_INITIAL_STEP,
...
#if !defined(IFX_CLOCK_DISABLE_RAMPCON)
    {IFXCLOCK_RAMP_OSC_UPPER_FREQ_LIMIT},
#endif
#if defined(IFX_CLOCK_DISABLE_RAMPCON)
    {
...
boolean IfxClock_configureCcuPll(const IfxClock_Config *pllCfg)
{
...

    {
        ...
        if ((pllCfg->pllInputClockSelection == IfxClock_PllInputClockSelection_fOsc0) || (pllCfg->pllInputClockSelection == IfxClock_PllInputClockSelection_fOsc_f))
        {
...
#endif /* #if (IFX_CLOCK_BYPASS_FOSC_F == 1) */
        ...
        (pllCfg->PerClockSourceSelect != IfxClock_PerClockSourceSelect_back))
    {
        ...
        {
            /* Step 1. PLL off */
            if (CLOCK_SYSPLLSTAT.B.PWRSTAT == 1)
            {
                if ((CLOCK_CCUSTAT.B.CLKSELS == 0x1) || (CLOCK_CCUSTAT.B.CLKSELS == 0x2))
                {
...
uint32 IfxClock_getFsi2Frequency(void)
{
    uint32               frequency;
    Ifx_CLOCK_SYSCCUCON0 sysccucon0 = CLOCK_SYSCCUCON0;

    if (sysccucon0.B.FSI2DIV == 0)
    {
        frequency = 0;
    }
    else
...
uint32 IfxClock_getRcbFrequency(void)
{
    uint32 rcbFrequency;
    uint32 sourceFrequency;
    sourceFrequency = IfxClock_getSysSourceFrequency();
    uint8  clkdiv[16] = {2, 2, 2, 3, 4, 5, 6, 6, 8, 8, 10, 10, 12, 12, 12, 15};

    if (CLOCK_SYSCCUCON0.B.LPDIV == 0)
    {
        /*Not in low power mode */
        rcbFrequency = sourceFrequency / clkdiv[CLOCK_SYSCCUCON1.B.RCBDIV];
    }
    else
    {
        rcbFrequency = sourceFrequency / 120;
    }
...
#if IFXCLOCK_IS_EXTCLK_FSI2_AVAILABLE
uint32 IfxClock_setFsi2Frequency(uint32 fsi2Freq)
{
    uint32               frequency = 0;
    ...
    IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, (fsi2Div <= IFXCLOCK_DIVIDER_1));
    ...
    frequency = IfxClock_getFsi2Frequency();
...
uint32 IfxClock_setGtmFrequency(uint32 gtmFreq)
{
    ...
    IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, (gtmDiv <= IFXCLOCK_DIVIDER_15));
...
#if IFXCLOCK_IS_EXTCLK_RCB_AVAILABLE
uint32 IfxClock_setRcbFrequency(uint32 rcbFreq)
{
    uint32               freq       = 0;
    uint32               source     = IfxClock_getSysSourceFrequency();
    Ifx_CLOCK_SYSCCUCON1 sysccucon1;
    uint32               rcbDiv     = (uint32)__roundf(source / rcbFreq);
    uint8                clkdiv[16] = {1, 1, 2, 3, 4, 5, 6, 6, 8, 8, 10, 10, 12, 12, 12, 15};
    IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, (rcbDiv <= IFXCLOCK_DIVIDER_15));
...
#if IFXCLOCK_IS_EXTCLK_RCB_AVAILABLE
			case 0x15:
            freq = IfxClock_getRcbFrequency();      //fRCB
            break;
#endif /* #if IFXCLOCK_IS_EXTCLK_RCB_AVAILABLE */
        case 0x16:
...
#if IFXCLOCK_IS_EXTCLK_GTM_AVAILABLE				
        case 0xf:
            freq = IfxClock_getGtmFrequency();      //	fGTM
            break;
#endif /* #if IFXCLOCK_IS_EXTCLK_GTM_AVAILABLE */
        case 0x18:
...
#if IFXCLOCK_IS_EXTCLK_AUDIO_AVAILABLE
		case 0x19:
            freq = IfxClock_getAudioPllFrequency(); //fPLLAUDIO
            break;
#endif /* #if IFXCLOCK_IS_EXTCLK_AUDIO_AVAILABLE */
        default:
            freq = 0;
...
#if IFXCLOCK_IS_EXTCLK_AUDIO_AVAILABLE	
		case 0x19:
            freq = IfxClock_getAudioSourceFrequency();                    //fAUDIO
            break;	
#endif /* #if IFXCLOCK_IS_EXTCLK_AUDIO_AVAILABLE */			
        default:
            freq = 0;
...
uint32 IfxClock_computeApllNDivRegVal(float32 targetFrequency)
{
    Ifx_CLOCK *clock = &MODULE_CLOCK;
    uint64     oscFreq;
    float32    nTotal;
    ...
    nTotal  = (targetFrequency * (clock->APLLCON1.B.KDIV + 1) * (clock->APLLCON0.B.PDIV + 1)) / oscFreq;

    /* Extract the fraction part */
    nFrac = nTotal - (uint32)nTotal;
    ...
    nFrac = IFXCLOCK_TWO_POWER_TWELVE_CONSTANT * nFrac;

    /* Now get the CLOCK_APLLCON2 value */
    nDivRegVal = ((nInt - 1) << IFX_CLOCK_APLLCON2_NDIV_OFF) | (uint32)nFrac;

    return (uint32)nDivRegVal;
}
#endif /* #if defined(IFX_CFG_CLOCK_APLL_FREQUENCY) */


#if IFXCLOCK_IS_EXTCLK_AUDIO_AVAILABLE
uint8 IfxClock_audioFreqTuning(uint32 nDiv)
{
    uint8  initError = 0U;
    uint32 timeoutCycleCount;
    uint32 quotient, remainder;
    uint32 targetNdiv = nDiv, currentNdiv;
    currentNdiv = CLOCK_APLLCON2.U;
    uint32 incNdiv, decNdiv;

    timeoutCycleCount = IFXCLOCK_CCUCON_LCK_BIT_TIMEOUT_COUNT;

    /* Check for APLLSTAT lock bit with a timeout of at least 100 micro seconds */
    while (CLOCK_APLLSTAT.B.LCK != 0U)
    {
        IFXCLOCK_LOOP_TIMEOUT_CHECK(timeoutCycleCount, initError);
    }

    if (targetNdiv > currentNdiv)
    {
        incNdiv   = targetNdiv - currentNdiv;
        quotient  = incNdiv / IFXCLOCK_APLLCON2_NDIVINT_DIVIDED_BY_TWO;
        remainder = incNdiv - IFXCLOCK_APLLCON2_NDIVINT_DIVIDED_BY_TWO * (incNdiv / IFXCLOCK_APLLCON2_NDIVINT_DIVIDED_BY_TWO);

        while (quotient--)
        {
            CLOCK_APLLCON2.U = CLOCK_APLLCON2.U + IFXCLOCK_APLLCON2_NDIVINT_DIVIDED_BY_TWO;
            /* Wait for 40 micro second delay for consecutive CPU write updates to the APLLCON1.NDIV_FRAC*/
            IfxClock_waitWithWdtService(IFXCLOCK_APLL_WAITTIME_FOR_NDIV_FRAC);
        }

        CLOCK_APLLCON2.U = CLOCK_APLLCON2.U + remainder;
    }

    else if (targetNdiv < currentNdiv)
    {
        decNdiv   = currentNdiv - targetNdiv;
        quotient  = decNdiv / IFXCLOCK_APLLCON2_NDIVINT_DIVIDED_BY_TWO;
        remainder = decNdiv - IFXCLOCK_APLLCON2_NDIVINT_DIVIDED_BY_TWO * (decNdiv / IFXCLOCK_APLLCON2_NDIVINT_DIVIDED_BY_TWO);
    ...
    {
        /*If targetNdiv == currentNdiv, skip the CLOCK_APLLCON1 update */
    }

    return initError;
}

float32 IfxClock_computeApllFreqFromNDivRegVal(uint32 nDivRegValue)
{
	Ifx_CLOCK *clock = &MODULE_CLOCK;
	float32 apllFreq;
	uint32 nInt;
	float32 nTotal, nFrac;
	uint64     oscFreq;

	nInt = (nDivRegValue>>IFX_CLOCK_APLLCON2_NDIV_OFF)+1;
	nFrac = (float32)(nDivRegValue & IFX_CLOCK_APLLCON2_NDIV_FRAC_MSK)/IFXCLOCK_TWO_POWER_TWELVE_CONSTANT;

	nTotal = nInt+nFrac;

	oscFreq = IfxClock_getOscFrequency();
	
	apllFreq = (float32)((nTotal*oscFreq)/((clock->APLLCON1.B.KDIV+1)*(clock->APLLCON0.B.PDIV + 1)));

    return apllFreq;
	
}

boolean IfxClock_disableAudioPll(void)
{
    uint8              initError         = 0;
    uint32             timeoutCycleCount = 0;
    ...
    ApllCon0.B.PLLPWR = 0;
    CLOCK_APLLCON0.U  = ApllCon0.U;

    return (boolean)initError;
}


boolean IfxClock_enableAudioPll(IfxClock_AudioConfig *pllCfg)
{
    uint32 timeoutCycleCount = 0U;
    uint8  initError         = 0U;
    timeoutCycleCount = IFXCLOCK_CCUCON_LCK_BIT_TIMEOUT_COUNT;
    ...
    /* Disable SMU Alarm Audio-PLL Out Of Range event */
    {
        boolean index = 0;
        IfxSmu_setSafetyAlarmAction(IfxSmu_Alarm_SF_CLOCKING_AudioPllCrystalClockFrequencyOutOfRange, IfxSmu_InternalAlarmAction_disabled, index);
    }

    if (pllCfg->pllInputClockSelection == IfxClock_PllInputClockSelection_fOsc0)
    {
        if ((CLOCK_OSCSTAT.B.AOWDS != 0U) && ((uint32)(pllCfg->xtalFrequency / 1000000U) >= 16U))
    ...
    /* Check if Audio PLL clockSrc is not equal to backup */
    if (pllCfg->AudioClockSourceSelect != IfxClock_AudioClockSourceSelect_back)

    {
        /* Configure the Audio PLL dividers (N, P, K) to desired frequency */

        if (pllCfg->AudioClockSourceSelect == IfxClock_AudioClockSourceSelect_pll)
        {
            ...
            /* Step 3. Configure K-divider in APLLCON1 register*/
            /* Step 3.a KDIV = 0x01  (APLLCON1.KDIV = 0x01) */

            CLOCK_APLLCON1.B.KDIV = 0x01;

            /* Step 3.b Check for CCU lock bit with a timeout of at least 100 micro seconds */
            timeoutCycleCount = IFXCLOCK_CCUSTAT_LCK_BIT_TIMEOUT_COUNT;
            ...
            /* Step 4. Power up Audio PLL - with PDIV and NDIV */

            Ifx_CLOCK_APLLCON2 audioPllCon2;

            audioPllCon2.U           = CLOCK_APLLCON2.U;
            audioPllCon2.B.NDIV      = pllCfg->audioPllConfig.nDivider;
            audioPllCon2.B.NDIV_FRAC = pllCfg->audioPllConfig.nFracDivider;

            CLOCK_APLLCON2.U         = audioPllCon2.U;

            Ifx_CLOCK_APLLCON0 audioPllCon0;

            audioPllCon0.U        = CLOCK_APLLCON0.U;
            audioPllCon0.B.PDIV   = pllCfg->audioPllConfig.pDivider;
            audioPllCon0.B.PLLPWR = 1;

            CLOCK_APLLCON0.U      = audioPllCon0.U;
            ...
            /* Step 6. Check if PLL is powered: APLLSTAT.PWRSTAT == 1 with a timeout of at least 100 micro seconds*/

            timeoutCycleCount = IFXCLOCK_CCUSTAT_LCK_BIT_TIMEOUT_COUNT;

            while (CLOCK_APLLSTAT.B.PWRSTAT != 1U)
            {
                IFXCLOCK_LOOP_TIMEOUT_CHECK(timeoutCycleCount, initError);
            }
            ...
            IfxClock_waitWithWdtService(IFXCLOCK_APLL_POWERUP_WAITTIME); /* 1 mS */

            /* Step 8. Check for APLL lock with a timeout of at least 100 micro seconds*/
            timeoutCycleCount = IFXCLOCK_CCUSTAT_LCK_BIT_TIMEOUT_COUNT;

            while (CLOCK_APLLSTAT.B.PLLLOCK != 1U)
            {
                IFXCLOCK_LOOP_TIMEOUT_CHECK(timeoutCycleCount, initError);
            }

            /* Step 9. Wait for 10 micro second and check for CCU lock: CLOCK_CCUSTAT.B.LCK == 0 with a timeout of at least 100 micro second */
            ...
            /* Step 10. Configure KDIV */
            /* Step 10.a Set KDIV to final value */

            Ifx_CLOCK_APLLCON1 audioPllCon1;

            audioPllCon1.U        = CLOCK_APLLCON1.U;
            audioPllCon1.B.KDIV   = pllCfg->audioPllConfig.kDivider;

            CLOCK_APLLCON1.B.KDIV = audioPllCon1.B.KDIV;

            /* Step 10.b Check for CCU lock: CLOCK_CCUSTAT.B.LCK == 0 with a timeout of at least 100 micro second */
            timeoutCycleCount = IFXCLOCK_CCUSTAT_LCK_BIT_TIMEOUT_COUNT;
    ...
    /* Clear SMU Alarm Audio-PLL Out Of Range event */
    {
        boolean index = 0;
        IfxSmu_smuSafeClearAlarmStatus(IfxSmu_Alarm_SF_CLOCKING_AudioPllCrystalClockFrequencyOutOfRange, index);
    }

    timeoutCycleCount = IFXCLOCK_PLL_LOCK_TIMEOUT_COUNT;

    while (CLOCK_APLLSTAT.B.PLLLOCK == 0U)
    {
        IFXCLOCK_LOOP_TIMEOUT_CHECK(timeoutCycleCount, initError);
    }
...
void IfxClock_audioInitConfig(IfxClock_AudioConfig *audioCfg)
{
    *audioCfg = IfxClock_defaultAudioClockConfig;
}


uint32 IfxClock_getAudioSourceFrequency(void)
{
    uint32 audioFreq = 0;

    switch (CLOCK_CCUSTAT.B.CLKSELA)
    {
    case IfxClock_AudioClockSourceSelect_pll:
        audioFreq = IfxClock_getAudioPllFrequency();
        break;

    case IfxClock_AudioClockSourceSelect_back:
        audioFreq = IfxClock_getEvrFrequency();
        break;
...
uint32 IfxClock_getAudioPllFrequency(void)
{
    Ifx_CLOCK *clock = &MODULE_CLOCK;
    float64    audioFrequency;
    uint64     oscFreq;
    oscFreq = IfxClock_getOscFrequency();

    uint32     nInt   = clock->APLLCON2.B.NDIV + 1;
    float32    nFrac  = clock->APLLCON2.B.NDIV_FRAC;
    float32    nTotal = nInt + (nFrac / IFXCLOCK_TWO_POWER_TWELVE_CONSTANT);

    audioFrequency = (oscFreq * nTotal) / ((clock->APLLCON0.B.PDIV + 1) * ((clock->APLLCON1.B.KDIV + 1)));
    audioFrequency = audioFrequency * clock->APLLSTAT.B.PLLLOCK * clock->APLLSTAT.B.PWRSTAT;

    return (uint32)audioFrequency;
}
#endif /* #if IFXCLOCK_IS_EXTCLK_AUDIO_AVAILABLE */
