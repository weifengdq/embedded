...
IfxXspi_Status IfxXspi_Spi_initModule(IfxXspi_Spi *xspi, const IfxXspi_Spi_Config *config)
{
    ...
    /* Configuring CTRLR2 - SPI Control Register */
    {
        Ifx_XSPI_CTRLR2 ctrlr2;
        ctrlr2.U          = xspiSFR->CTRLR2.U;
        ctrlr2.B.PORTSEL  = config->portSelect;
        xspiSFR->CTRLR2.U = ctrlr2.U;
    }

    {
        if (xspiSFR->CTRLR2.B.PORTSEL == IfxXspi_SelectPort_16)
        {
            /* For PORT 16 highspeed mode Fxspi clock is multipled by 2 hence divider value need not be right sifted by 1 */
            xspiSFR->BAUDR.B.SCKDV = (IfxClock_getXspiFrequency() / config->timing.baudrate);
        }
        else
        {
            /* Fsclk_out =	Fssi_clk/SCKDV where SCKDV is any even value between 2 and 65534 */
            xspiSFR->BAUDR.B.SCKDV = ((IfxClock_getXspiFrequency() / config->timing.baudrate) >> 1);   /* Configuring Baud Rate Select */
        }
    }
    ...
    if ((config->interruptConfig.fifoPriority != 0) || (config->interruptConfig.fifoIntTypeOfService == IfxSrc_Tos_dma0))
#endif
    {
...
#else
            .fifoIntTypeOfService                         = IfxSrc_Tos_ppu, /* Fifo type of interrupt service */
            .errorIntTypeOfService                        = IfxSrc_Tos_ppu, /* Error type of interrupt service */
#endif            
            .interruptMask.doneInterruptMask              = IfxXspi_InterruptMask_Mask,
            .interruptMask.errorInterruptMask             = IfxXspi_InterruptMask_Mask,
            .interruptMask.txFifoUnderFlowInterruptMask   = IfxXspi_InterruptMask_Mask,
            .interruptMask.xipRxFifoOverFlowInterruptMask = IfxXspi_InterruptMask_Mask,
            .interruptMask.multiMasterInterruptMask       = IfxXspi_InterruptMask_Mask,
            .interruptMask.rxFifoFullInterruptMask        = IfxXspi_InterruptMask_Mask,
            .interruptMask.rxFifoOverFlowInterruptMask    = IfxXspi_InterruptMask_Mask,
            .interruptMask.rxFifoUnderFlowInterruptMask   = IfxXspi_InterruptMask_Mask,
            .interruptMask.txFifoOverFlowInterruptMask    = IfxXspi_InterruptMask_Mask,
            .interruptMask.txFifoEmptyInterruptMask       = IfxXspi_InterruptMask_Mask,
        },

        .dmaConfig       = &dmaDefaultConfig,
        .portSelect      = IfxXspi_SelectPort_16,                /* Port 16 Select by default */
        .enhancedControl = {
...
IfxXspi_Spi_Status IfxXspi_Spi_dmaExchange(IfxXspi_Spi *xspi, IfxXspi_Spi_DmaJobConfig *dmaJob)
{
    ...
    if (xspi->transferInProgress == 0)
    {
        ...
        case IfxXspi_TransferMode_EepromRead:
            ...
            xspi->xspi->SPI_CTRLR0.B.SPI_RXDS_SIG_EN = dmaJob->rxdsSignalEnable;
#if IFXXSPI_INTERRUPT_CFG
            // configure intr masks
            xspi->xspi->IMR.B.RXFIM = IfxXspi_InterruptMask_Mask;
            xspi->xspi->IMR.B.RXUIM = IfxXspi_InterruptMask_UnMask;
            xspi->xspi->IMR.B.RXOIM = IfxXspi_InterruptMask_UnMask;
        ...
        case IfxXspi_TransferMode_Tx:
            ...
            xspi->xspi->IMR.B.TXOIM = IfxXspi_InterruptMask_UnMask;
            xspi->xspi->IMR.B.TXEIM = IfxXspi_InterruptMask_Mask;
            xspi->xspi->IMR.B.TXUIM = IfxXspi_InterruptMask_UnMask;
...
boolean IfxXspi_Spi_exchange(IfxXspi_Spi *xspi, IfxXspi_Spi_CpuJobConfig *jobConfig)
{
    ...
    if (xspi->transferInProgress == 0)
    {
        ...
        {
        case IfxXspi_TransferMode_TxRx:
            xspi->xspi->IMR.B.RXFIM  = IfxXspi_InterruptMask_UnMask;
            xspi->xspi->IMR.B.RXUIM  = IfxXspi_InterruptMask_UnMask;
            xspi->xspi->IMR.B.RXOIM  = IfxXspi_InterruptMask_UnMask;
            xspi->xspi->IMR.B.TXOIM  = IfxXspi_InterruptMask_UnMask;
            xspi->xspi->IMR.B.TXEIM  = IfxXspi_InterruptMask_UnMask;
            xspi->xspi->IMR.B.TXUIM  = IfxXspi_InterruptMask_UnMask;
...
IfxXspi_Status IfxXspi_Spi_enhancedDataTransmit(Ifx_XSPI *xspi, IfxXspi_Spi_EnhancedModeConfig *config)
{
    ...
    xspi->TXFTLR.B.TXFTHR = IfxXspi_Spi_computeTXStartFIFOLevel(config->instructionLength, config->addrLength, txCount);
#endif
    ...
    xspi->CTRLR1.B.NDF                 = (txCount == 0) ? 0 : (txCount - 1);     /* XSPI_ILLD_PATCH */
...
void IfxXspi_Spi_xipDataRead(Ifx_XSPI *xspi, IfxXspi_Spi_XipModeConfig *config)
{
    ...
    xspi->SPI_CTRLR0.B.SPI_DM_EN   = config->spiDataMaskEnable;
    xspi->SPI_CTRLR0.B.CLK_STRETCH_EN = config->spiClockStretchEnable;
...
#else
            xspi->AXIAR0.B.AXIAR_0_31          = (uint32)(config->data);
#endif
...
void IfxXspi_Spi_XipDataWrite(Ifx_XSPI *xspi, IfxXspi_Spi_XipModeConfig *config)
{
    xspi->SSIENR.B.SSIC_EN     = IfxXspi_Dwc_Mode_Disable; //Disable DWC_SSI
// Configure access to XIP regions
    xspi->RGN[0].ACCEN.RGNLA.U = IFXXSPI_XIP_REGION0_LOWER;
...
void IfxXspi_Spi_switchSpiProtocol(Ifx_XSPI *xspi, IfxXspi_Spi_spiProtocolSwitch *config)
{
    ...
    if (xspi->CTRLR2.B.PORTSEL == IfxXspi_SelectPort_16)
    {
        /* For PORT 16 highspeed mode Fxspi clock is multipled by 2 hence divider value need not be right sifted by 1 */
        xspi->BAUDR.B.SCKDV = (IfxClock_getXspiFrequency() / config->baudrate);
    }
    else
    {
        /* Fsclk_out =	Fssi_clk/SCKDV where SCKDV is any even value between 2 and 65534 */
        xspi->BAUDR.B.SCKDV = ((IfxClock_getXspiFrequency() / config->baudrate) >> 1); /* Configuring Baud Rate Select */
    }
...
uint8 IfxXspi_Spi_computeTXStartFIFOLevel(IfxXspi_InstructionLength instLength, IfxXspi_AddressLength addrLength, uint32 txCount)
{
    uint32 offset           = 0;
    ...
    if (txCount != 0)
    {
        if (instLength != IfxXspi_InstructionLength_L0)
        {
            offset++;
        }

        if (addrLength != IfxXspi_AddressLength_L0)
        {
            offset++;

            if (addrLength > IfxXspi_AddressLength_L32)
            {
                offset++;
            }
        }
        ...
        txFifoStartLevel = __min(offset, IFXXSPI_MAX_TXFIFO_LEVEL);
    }
    else
    {
        txFifoStartLevel = 0;
    ...
    return txFifoStartLevel;
}
void IfxXspi_Spi_clearTransferDoneInterrupt(Ifx_XSPI *xspi)
{
    xspi->DONECR.B.DONECR = 1; /* Clear DONECR interrupt */
}
void IfxXspi_Spi_clearTXFIFOErrorInterrupt(Ifx_XSPI *xspi)
{
    xspi->TXEICR.B.TXEICR = 1; /* Clear TXEICR interrupt */
}
void IfxXspi_Spi_clearRXFIFOOverflowInterrupt(Ifx_XSPI *xspi)
{
    xspi->RXOICR.B.RXOICR = 1; /* Clear RXOICR interrupt */
}
void IfxXspi_Spi_clearRXFIFOUnderflowInterrupt(Ifx_XSPI *xspi)
{
    xspi->RXUICR.B.RXUICR = 1; /* Clear RXUICR interrupt */
}
void IfxXspi_Spi_clearMasterContentionInterrupt(Ifx_XSPI *xspi)
{
    xspi->MSTICR.B.MSTICR = 1; /* Clear MSTICR interrupt */
}
void IfxXspi_Spi_clearAXIMasterErrorInterrupt(Ifx_XSPI *xspi)
{
    xspi->AXIECR.B.AXIECR = 1; /* Clear AXIECR interrupt */
}
void IfxXspi_Spi_clearInterrupt(Ifx_XSPI *xspi)
{
    xspi->ICR.B.ICR = 1; /* Clear all interrupts */
}
