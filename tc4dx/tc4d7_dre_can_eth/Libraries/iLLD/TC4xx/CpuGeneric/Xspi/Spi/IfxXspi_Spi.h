...
typedef struct
{
    ...
    IfxXspi_ClockLoopBack      clockLoopBack;            /**< \brief Clock Loopback enable */
} IfxXspi_Spi_Control;
...
typedef struct
{
    ...
    IfxXspi_InstructionLength instLength;               /**< \brief DMA Instruction Length */
    uint32                    waitCycles;               /**< \brief Wait cycle */
    IfxXspi_TransferMode      transferMode;             /**< \brief Transfer Mode */
    IfxXspi_SlaveSelectEnable slaveSelectEnable;        /**< \brief xspi transfer Slave select enable */
    boolean                   rxdsSignalEnable;        /**< \brief Enable/Disable RXDS signal */
...
typedef struct
{
    uint16                    instruction;               /**< \brief  */
    IfxXspi_InstructionLength instructionLength;         /**< \brief  */
    uint64                    address;                   /**< \brief Clock Phase */
    IfxXspi_AddressLength     addrLength;                /**< \brief defines Length of Address to be transmitted */
    uint32                    waitCycles;                /**< \brief number of wait cycles to received data from xSPI flash */
    uint32                    dataCount;                 /**< \brief no of data frames to transmit/receive */
    uint8                    *txData;                    /**< \brief Pointer to transmit data */
    uint8                    *rxData;                    /**< \brief Pointer to received data */
    boolean                   enableAsymBaudrate;        /**< \brief Enable asymetetric baudrate */
    float32                   buadrate;                  /**< \brief baudrate */
    boolean                   spiRxdsSignalEnable;       /**< \brief Enable rxds signaling during address and command phase of Hypebus transfer */
    IfxXspi_SlaveSelectEnable slaveSelectEnable;         /**< \brief xspi transfer Slave select enable */
...
typedef struct
{
    ...
    IfxXspi_SelectPort  selPort;       /**< \brief Configure the PORT Selection */
...
typedef struct
{
    ...
    boolean                   spiDataMaskEnable;             /**< \brief Enable/Disable SPI data masking */
    boolean                   spiClockStretchEnable;         /**< \brief Enable/Disable SPI clock stretching */
...
typedef union
{
    ...
    uint16 *u16ptr;
    uint32 *u32ptr;
} IfxXspi_Spi_bufferPointers;

/** \brief Structure for Protocol switch config
 */
typedef struct
{
    IfxXspi_SpiFrameFormat spiFrameFormat;
    IfxXspi_TransferFormat transferFormat;
    boolean                spiInstDdrEnable;
    boolean                spiDdrEnable;             /**< \brief SPI DDR Enable bit. This will enable Dual-data rate transfers in Dual/Quad/Octal frame formats of SPI */
    IfxXspi_DataframeSize  dataFrameSize;            /**< \brief Data frame size for XIP transfer */
    boolean                spiRxdsEnable;
    IfxXspi_Hyperbus       hyperbusEnable;           /**< \brief HyperBus Enable/Disable */
    IfxXspi_ClockLoopBack  clockLoopBack;
    uint32                 baudrate;
    boolean                ddrTxDriveEdge;           /**< \brief DDR Transmit Drive Edge */
    uint8                  rxDataSampleDelay;        /**< \brief Receive Data (rxd) Sample Delay */
    boolean                rxDataSamplingEdge;       /**< \brief Receive Data (rxd) Sampling Edge */
    boolean                spiDataMaskEnable;       /**< \brief Enable/Disable SPI data masking */
} IfxXspi_Spi_spiProtocolSwitch;
...
/** \brief Initializes the XSPI module with the provided configuration.
 * \param xspi Pointer to the Ifx_XSPI handler structure.
...
/** \brief Initializes the xSPI module configuration with default settings.
 * \param config Pointer to the IfxXspi_Spi_Config structure to be initialized.
 * \param xspi xspi Pointer to the Ifx_XSPI handler structure.
 * \return None
 */
IFX_EXTERN void IfxXspi_Spi_initModuleConfig(IfxXspi_Spi_Config *config, Ifx_XSPI *xspi);

/** \brief Initializes the SPI transfer configuration for the XSPI module.
 * \param xspi xspi Pointer to the Ifx_XSPI handler structure
 * \param config Pointer to the IfxXspi_Spi_initTransferConfig structure to be initialized.
 * \return None
 */
...
/** \brief Retrieves the raw interrupt status of the SPI module.
 * \param xspi Pointer to the Ifx_XSPI handler structure.
 * \param rawInterruptStatus Pointer to the structure where the raw interrupt statuses will be stored.
 * \return None
 */
IFX_EXTERN void IfxXspi_Spi_getRawInterruptStatus(IfxXspi_Spi *xspi, IfxXspi_Spi_RawInterruptStatus *rawInterruptStatus);

/** \brief Interrupt Service Routine (ISR) for handling SPI transmit operations.
 * \param xspi Pointer to the Ifx_XSPI handler structure.
 * \return None
 */
IFX_EXTERN void IfxXspi_Spi_isrTransmit(IfxXspi_Spi *xspi);

/** \brief Interrupt Service Routine for DMA Receive in SPI module.
 * \param xspi Pointer to the Ifx_XSPI handler structure.
 * \return Zero : All interrupts are cleared, Non Zero : All interrupts are not cleared
 */
...
/** \brief Module Handle
 * \param xspi Pointer to the Ifx_XSPI handler structure.
 * \return IfxXspi_BusStatus_busy Indicates the bus is currently busy.
 */
...
/** \brief Performs a SPI DMA exchange operation.
 * \param xspi Pointer to the Ifx_XSPI handler structure.
 * \param dmaJob Pointer to the DMA job configuration structure containing transfer parameters.
 * \return IfxXspi_Spi_Status_success Operation completed successfully.
 */
IFX_EXTERN IfxXspi_Spi_Status IfxXspi_Spi_dmaExchange(IfxXspi_Spi *xspi, IfxXspi_Spi_DmaJobConfig *dmaJob);

/** \brief Fills the transmit buffer for the SPI interface.
 * \param xspi Pointer to the Ifx_XSPI handler structure.
 * \return None
 */
IFX_EXTERN void IfxXspi_Spi_fillTxBuffer(IfxXspi_Spi *xspi);

/** \brief Performs an SPI exchange operation, handling different transfer modes and initializing the SPI controller.
 * \param xspi Pointer to the Ifx_XSPI handler structure.
 * \param jobConfig Pointer to the IfxXspi_Spi_CpuJobConfig structure containing transfer configuration details.
 * \return TRUE Transfer initialization was successful.
 */
...
/** \brief Triggers an XIP transfer using the SPI interface (single / continuous mode)
 * \param xspi Pointer to the Ifx_XSPI handler structure.
 * \param config Pointer to the IfxXspi_Spi_XipConfig structure containing transfer configuration.
 * \return 0 Success.
 */
 ...
 * \param config Pointer to the SPI read/write configuration structure.
 * \param address The target memory address to read data from.
 * \param data Pointer to store the read data value.
 * \return TRUE Successful read operation.
 */
 ...
 * \param xspi Pointer to the Ifx_XSPI structure representing the SPI instance.
 * \param config configuration structure of the Standard transfer mode
 * \return IfxXspi_Status_success The SPI data exchange was successful.
 */
 ...
 */
IFX_EXTERN IfxXspi_Status IfxXspi_Spi_enhancedDataTransmit(Ifx_XSPI *xspi, IfxXspi_Spi_EnhancedModeConfig *config);
 ...
 * \param xspi Pointer to the Ifx_XSPI structure representing the SPI instance.
 * \param pins Pointer to the xSPI GPIO pin configuration structure.
 * \return Returns None
 */
 ...
 * \param xspi Pointer to the Ifx_XSPI structure representing the SPI instance.
 * \param config Pointer to the configuration structure for the DMA exchange.
 * \return Returns Status
 */
IFX_EXTERN IfxXspi_Status IfxXspi_Spi_dmaEnhancedDataExchange(Ifx_XSPI *xspi, IfxXspi_Spi_DmaExchangeConfig *config);

#if IFXXSPI_XIPDATAWRITE_SUPPORT
/** \brief API to enable fast switch to different SPI protocol
 * \param xspi xSPI SFR Module Handle
 * \param config protocol switch configurations
 * \return None
 */
IFX_EXTERN void IfxXspi_Spi_XipDataWrite(Ifx_XSPI *xspi, IfxXspi_Spi_XipModeConfig *config);
#endif /* #if IFXXSPI_XIPDATAWRITE_SUPPORT */

/** \brief Write data up to 32 bits
 * \param instLength instruction length
 * \param addrLength address length
 * \param txCount tx number of count
 * \return TX FIFO level value
 */
IFX_EXTERN uint8 IfxXspi_Spi_computeTXStartFIFOLevel(IfxXspi_InstructionLength instLength, IfxXspi_AddressLength addrLength, uint32 txCount);

/**
 * \brief Clears the Transfer Done interrupt status flag.
 * \param xspi Pointer to the XSPI module.
 */
IFX_EXTERN void IfxXspi_Spi_clearTransferDoneInterrupt(Ifx_XSPI *xspi);

/**
 * \brief Clears the TX FIFO Error interrupt status flag.
 * \param xspi Pointer to the XSPI module.
 */
IFX_EXTERN void IfxXspi_Spi_clearTXFIFOErrorInterrupt(Ifx_XSPI *xspi);

/**
 * \brief Clears the RX FIFO Overflow interrupt status flag.
 * \param xspi Pointer to the XSPI module.
 */
IFX_EXTERN void IfxXspi_Spi_clearRXFIFOOverflowInterrupt(Ifx_XSPI *xspi);

/**
 * \brief Clears the RX FIFO Underflow interrupt status flag.
 * \param xspi Pointer to the XSPI module.
 */
IFX_EXTERN void IfxXspi_Spi_clearRXFIFOUnderflowInterrupt(Ifx_XSPI *xspi);
/**
 * \brief Clears the Master Contention interrupt status flag.
 * \param xspi Pointer to the XSPI module.
 */
IFX_EXTERN void IfxXspi_Spi_clearMasterContentionInterrupt(Ifx_XSPI *xspi);

/**
 * \brief Clears the AXI Master Error interrupt status flag.
 * \param xspi Pointer to the XSPI module.
 */
IFX_EXTERN void IfxXspi_Spi_clearAXIMasterErrorInterrupt(Ifx_XSPI *xspi);

/**
 * \brief Clears all interrupt status flags.
 * \param xspi Pointer to the XSPI module.
 */
IFX_EXTERN void IfxXspi_Spi_clearInterrupt(Ifx_XSPI *xspi);
...
/** \brief Enables or disables the clock stretch feature for the SPI controller.
 * \param xspi pointer to XSPI registers
 * \param enableClockStretch TRUE: Enables clock stretching FALSE: Disables clock stretching feature
 * \return None
 */
IFX_EXTERN void IfxXspi_Spi_enableClockStretch(Ifx_XSPI *xspi, boolean enableClockStretch);
 ...
 * \return None
 */
IFX_EXTERN void IfxXspi_Spi_switchSpiProtocol(Ifx_XSPI *xspi, IfxXspi_Spi_spiProtocolSwitch *config);
#endif /* IFXXSPI_SPI_H */
