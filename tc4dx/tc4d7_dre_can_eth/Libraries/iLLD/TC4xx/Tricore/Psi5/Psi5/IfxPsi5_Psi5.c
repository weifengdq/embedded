...
{
    uint32 result;
    uint32 fPsi5 = IfxClock_getSpbFrequency();
...
boolean IfxPsi5_Psi5_initChannel(IfxPsi5_Psi5_Channel *channel, const IfxPsi5_Psi5_ChannelConfig *config)
{
    ...
    tempPGC.B.TRIGSEL = config->pulseGeneration.triggersource;

    switch (config->pulseGeneration.periodicOrExternalOrBypass)
    {
    case IfxPsi5_TriggerType_periodic:
        tempPGC.B.PTE = IfxPsi5_PeriodicTrigger_enabled;
        tempPGC.B.ETE = IfxPsi5_ExternalTriggerLine_disabled;
        tempPGC.B.BYP = IfxPsi5_Externaltrigger_Bypass_disabled;
        break;

    case IfxPsi5_TriggerType_external:
        tempPGC.B.PTE = IfxPsi5_PeriodicTrigger_disabled;
        tempPGC.B.ETE = IfxPsi5_ExternalTriggerLine_enabled;
        tempPGC.B.BYP = IfxPsi5_Externaltrigger_Bypass_disabled;
        break;
    ...
    case IfxPsi5_TriggerType_bypass:
        tempPGC.B.PTE = IfxPsi5_PeriodicTrigger_disabled;
        tempPGC.B.ETE = IfxPsi5_ExternalTriggerLine_disabled;
        tempPGC.B.BYP = IfxPsi5_Externaltrigger_Bypass_enabled;
        break;
    }
...
void IfxPsi5_Psi5_initChannelConfig(IfxPsi5_Psi5_ChannelConfig *config, IfxPsi5_Psi5 *psi5)
{
    IfxPsi5_Psi5_ChannelConfig IfxPsi5_Psi5_defaultChannelConfig = {
        ...
        .pulseGeneration = {
            ...
            .externalTimeBaseSelect     = IfxPsi5_ExternalTrigger_0,
            .periodicOrExternalOrBypass = IfxPsi5_TriggerType_periodic,
            .externalTriggerSelect      = IfxPsi5_ExternalTrigger_0,
            .blankoutTime               = 5,
            .triggersource              = IfxPsi5_TriggerSelect_gtmTrigger
        },
        .channelTrigger                         = {
...
#if (IFX_PROT_ENABLED == 1U)
    IfxApProt_setState((Ifx_PROT_PROT *)&psi5SFR->PROTE, IfxApProt_State_config);
#endif
...
#if (IFX_PROT_ENABLED == 1U)
    IfxApProt_setState((Ifx_PROT_PROT *)&psi5SFR->PROTE, IfxApProt_State_run);
#endif
...
void IfxPsi5_Psi5_initModuleConfig(IfxPsi5_Psi5_Config *config, Ifx_PSI5 *psi5)
{
    ...
    config->timestampCounterA.externalTimeBaseSelect = IfxPsi5_ExternalTrigger_0;
    config->timestampCounterA.timeBaseSelect         = IfxPsi5_TimeBase_internal;
    config->timestampCounterB.externalTimeBaseSelect = IfxPsi5_ExternalTrigger_0;
    config->timestampCounterB.timeBaseSelect         = IfxPsi5_TimeBase_internal;
    config->timestampCounterC.externalTimeBaseSelect = IfxPsi5_ExternalTrigger_0;
...
uint32 IfxPsi5_Psi5_initializeClock(Ifx_PSI5 *psi5, const IfxPsi5_Psi5_Clock *clock)
{
    ...
    {
        fInput = IfxClock_getSpbFrequency();
    }
    else
    {
...
void IfxPsi5_Psi5_resetModule(Ifx_PSI5 *psi5)
{
    ...
    psi5->RST.CTRLB.B.KRST = 1;      /* Only if both Kernel reset bits are set a reset is executed */
    psi5->RST.CTRLA.B.KRST = 1;

    while (psi5->RST.STAT.B.KRST == 0)
    {
