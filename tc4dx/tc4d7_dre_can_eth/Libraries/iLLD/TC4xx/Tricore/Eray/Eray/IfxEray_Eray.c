...
/** \brief Initializes the message RAM.
 * \param eray pointer to ERAY Module handle.
...
void IfxEray_Eray_initModule(IfxEray_Eray *eray, const IfxEray_Eray_Config *config)
{
    ...
    // Enable MTU clock
    {
        IfxVmt_MbistSel   mbistSel    = IfxVmt_MbistSel_eray0Obf;
        volatile Ifx_VMT *vmtInstance = IfxVmt_getDmtuAddress(mbistSel);
        IfxVmt_enableModule(vmtInstance);
    }
    // clear RAMS
    {
        if (eraySFR == &MODULE_ERAY0)
        {
            IfxVmt_clearSram(IfxVmt_MbistSel_eray0Obf);
            IfxVmt_clearSram(IfxVmt_MbistSel_eray0IbfTbf);
            IfxVmt_clearSram(IfxVmt_MbistSel_eray0Mbf);
        }
		
#if IFXERAY_NUM_MODULES > 1
        if (eraySFR == &MODULE_ERAY1)
        {
            IfxVmt_clearSram(IfxVmt_MbistSel_eray1Obf);
            IfxVmt_clearSram(IfxVmt_MbistSel_eray1IbfTbf);
            IfxVmt_clearSram(IfxVmt_MbistSel_eray1Mbf);
        }
#endif	/* #if IFXERAY_NUM_MODULES > 1 */
 ...
 * this function is called from */
    {
        ...
        if ((config->interrupt.int0Priority != 0) || (config->interrupt.int0IsrProvider == IfxSrc_Tos_dma0) || (config->interrupt.int0IsrProvider == IfxSrc_Tos_dma1))
#else

        if ((config->interrupt.int0Priority != 0) || (config->interrupt.int0IsrProvider == IfxSrc_Tos_dma0))
#endif
        {
            volatile Ifx_SRC_SRCR *src = IfxEray_getInterruptLine0SrcPtr(eraySFR);
            IfxSrc_init(src, config->interrupt.int0IsrProvider, config->interrupt.int0Priority, config->interrupt.vmId);
            IfxSrc_enable(src);
        }

#if (IFXDMA_NUM_MODULES > 1U)

        if ((config->interrupt.int1Priority != 0) || (config->interrupt.int1IsrProvider == IfxSrc_Tos_dma0) || (config->interrupt.int1IsrProvider == IfxSrc_Tos_dma1))
#else

        if ((config->interrupt.int1Priority != 0) || (config->interrupt.int1IsrProvider == IfxSrc_Tos_dma0))
#endif
        {
            volatile Ifx_SRC_SRCR *src = IfxEray_getInterruptLine1SrcPtr(eraySFR);
            IfxSrc_init(src, config->interrupt.int1IsrProvider, config->interrupt.int1Priority, config->interrupt.vmId);
            IfxSrc_enable(src);
        }

#if (IFXDMA_NUM_MODULES > 1U)

        if ((config->interrupt.tint0Priority != 0) || (config->interrupt.tint0IsrProvider == IfxSrc_Tos_dma0) || (config->interrupt.tint0IsrProvider == IfxSrc_Tos_dma1))
#else

        if ((config->interrupt.tint0Priority != 0) || (config->interrupt.tint0IsrProvider == IfxSrc_Tos_dma0))
#endif
        {
            volatile Ifx_SRC_SRCR *src = IfxEray_getTimerInterrupt0SrcPtr(eraySFR);
            IfxSrc_init(src, config->interrupt.tint0IsrProvider, config->interrupt.tint0Priority, config->interrupt.vmId);
            IfxSrc_enable(src);
        }

#if (IFXDMA_NUM_MODULES > 1U)

        if ((config->interrupt.tint1Priority != 0) || (config->interrupt.tint1IsrProvider == IfxSrc_Tos_dma0) || (config->interrupt.tint1IsrProvider == IfxSrc_Tos_dma1))
#else

        if ((config->interrupt.tint1Priority != 0) || (config->interrupt.tint1IsrProvider == IfxSrc_Tos_dma0))
#endif
        {
            volatile Ifx_SRC_SRCR *src = IfxEray_getTimerInterrupt1SrcPtr(eraySFR);
            IfxSrc_init(src, config->interrupt.tint1IsrProvider, config->interrupt.tint1Priority, config->interrupt.vmId);
            IfxSrc_enable(src);
        }

#if (IFXDMA_NUM_MODULES > 1U)

        if ((config->interrupt.mbsc0Priority != 0) || (config->interrupt.mbsc0IsrProvider == IfxSrc_Tos_dma0) || (config->interrupt.mbsc0IsrProvider == IfxSrc_Tos_dma1))
#else

        if ((config->interrupt.mbsc0Priority != 0) || (config->interrupt.mbsc0IsrProvider == IfxSrc_Tos_dma0))
#endif
        {
            volatile Ifx_SRC_SRCR *src = IfxEray_getMessageBufferStatus0SrcPtr(eraySFR);
            IfxSrc_init(src, config->interrupt.mbsc0IsrProvider, config->interrupt.mbsc0Priority, config->interrupt.vmId);
            IfxSrc_enable(src);
        }

#if (IFXDMA_NUM_MODULES > 1U)

        if ((config->interrupt.mbsc1Priority != 0) || (config->interrupt.mbsc1IsrProvider == IfxSrc_Tos_dma0) || (config->interrupt.mbsc1IsrProvider == IfxSrc_Tos_dma1))
#else

        if ((config->interrupt.mbsc1Priority != 0) || (config->interrupt.mbsc1IsrProvider == IfxSrc_Tos_dma0))
#endif
        {
            volatile Ifx_SRC_SRCR *src = IfxEray_getMessageBufferStatus1SrcPtr(eraySFR);
            IfxSrc_init(src, config->interrupt.mbsc1IsrProvider, config->interrupt.mbsc1Priority, config->interrupt.vmId);
            IfxSrc_enable(src);
        }

#if (IFXDMA_NUM_MODULES > 1U)

        if ((config->interrupt.obusyPriority != 0) || (config->interrupt.ibusyIsrProvider == IfxSrc_Tos_dma0) || (config->interrupt.ibusyIsrProvider == IfxSrc_Tos_dma1))
#else

        if ((config->interrupt.obusyPriority != 0) || (config->interrupt.ibusyIsrProvider == IfxSrc_Tos_dma0))
#endif
        {
            volatile Ifx_SRC_SRCR *src = IfxEray_getInputBufferBusySrcPtr(eraySFR);
            IfxSrc_init(src, config->interrupt.ibusyIsrProvider, config->interrupt.ibusyPriority, config->interrupt.vmId);
            IfxSrc_enable(src);
        }

#if (IFXDMA_NUM_MODULES > 1U)

        if ((config->interrupt.obusyPriority != 0) || (config->interrupt.obusyIsrProvider == IfxSrc_Tos_dma0) || (config->interrupt.obusyIsrProvider == IfxSrc_Tos_dma1))
#else

        if ((config->interrupt.obusyPriority != 0) || (config->interrupt.obusyIsrProvider == IfxSrc_Tos_dma0))
#endif
        {
            volatile Ifx_SRC_SRCR *src = IfxEray_getOutputBufferBusySrcPtr(eraySFR);
            IfxSrc_init(src, config->interrupt.obusyIsrProvider, config->interrupt.obusyPriority, config->interrupt.vmId);
            IfxSrc_enable(src);
        }

#if (IFXDMA_NUM_MODULES > 1U)

        if ((config->interrupt.ndat0Priority != 0) || (config->interrupt.ndat0IsrProvider == IfxSrc_Tos_dma0) || (config->interrupt.ndat0IsrProvider == IfxSrc_Tos_dma1))
#else

        if ((config->interrupt.ndat0Priority != 0) || (config->interrupt.ndat0IsrProvider == IfxSrc_Tos_dma0))
#endif
        {
            volatile Ifx_SRC_SRCR *src = IfxEray_getNewDataInterrupt0SrcPtr(eraySFR);
            IfxSrc_init(src, config->interrupt.ndat0IsrProvider, config->interrupt.ndat0Priority, config->interrupt.vmId);
            IfxSrc_enable(src);
        }

#if (IFXDMA_NUM_MODULES > 1U)

        if ((config->interrupt.ndat1Priority != 0) || (config->interrupt.ndat1IsrProvider == IfxSrc_Tos_dma0) || (config->interrupt.ndat1IsrProvider == IfxSrc_Tos_dma1))
#else

        if ((config->interrupt.ndat1Priority != 0) || (config->interrupt.ndat1IsrProvider == IfxSrc_Tos_dma0))
#endif
        {
            volatile Ifx_SRC_SRCR *src = IfxEray_getNewDataInterrupt1SrcPtr(eraySFR);
            IfxSrc_init(src, config->interrupt.ndat1IsrProvider, config->interrupt.ndat1Priority, config->interrupt.vmId);
            IfxSrc_enable(src);
        }
    }
}
