...
void IfxEray_resetModule(Ifx_ERAY *eray)
{
    ...
    IfxApProt_setState((Ifx_PROT_PROT *)&eray->PROTE, IfxApProt_State_config);
#endif
    eray->RST.CTRLB.B.KRST = 1;      /* Only if both Kernel reset bits are set a reset is executed */
    eray->RST.CTRLA.B.KRST = 1;

    while (eray->RST.STAT.B.KRST == 0)
    {
        /* Wait until reset is executed */
    }

    eray->RST.CTRLB.B.STATCLR = 1;     /* Clear Kernel reset status bit */

    while (eray->RST.STAT.B.KRST == 1) /* Wait until KRST is cleared, only after this reset sequence is completed */
    {}

#if (IFX_PROT_ENABLED == 1U)
    IfxApProt_setState((Ifx_PROT_PROT *)&eray->PROTE, IfxApProt_State_run);
#endif
}
...
{
    uint8           ix      = ndat >> 5;
    uint32          mask    = 1 << (ndat & 31); // uint32  mask = 1 << (ndat % 32) ;
...
void IfxEray_initApConfig(IfxEray_ApConfig *config)
{
...
void IfxEray_initAp(Ifx_ERAY *eray, IfxEray_ApConfig *config)
{
    /* Initialize the PROTs */
    IfxApProt_init((Ifx_PROT_PROT *)&eray->PROTE, &config->proteConfig);
    IfxApProt_init((Ifx_PROT_PROT *)&eray->PROTSE, &config->protseConfig);

    /* Change the state to CONFIG, Configure APU and set PROT state back to RUN */
    /* Initialize the APU */
    IfxApProt_setState((Ifx_PROT_PROT *)&eray->PROTSE, IfxApProt_State_config);
    IfxApApu_init((Ifx_ACCEN_ACCEN *)&eray->ACCEN, &config->apuConfig);
    IfxApProt_setState((Ifx_PROT_PROT *)&eray->PROTSE, IfxApProt_State_run);
}


void IfxEray_configureAccessToErays(IfxApApu_ApuConfig *apConfig)
{
    ...
    for (index = 0; index < IFXERAY_NUM_MODULES; index++)
    {
        ...
        Ifx_ERAY *eray = (Ifx_ERAY *)(((uint32)&MODULE_ERAY0) + index * ((uint32)&MODULE_ERAY1 - (uint32)&MODULE_ERAY0));

        IfxApApu_init((Ifx_ACCEN_ACCEN *)&eray->ACCEN, apConfig);
    }
#else
    Ifx_ERAY *eray = (Ifx_ERAY *)((uint32)&MODULE_ERAY0);
    IfxApApu_init((Ifx_ACCEN_ACCEN *)&eray->ACCEN, apConfig);	
