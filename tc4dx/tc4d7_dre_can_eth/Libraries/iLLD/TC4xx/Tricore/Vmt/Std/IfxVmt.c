...
uint8 IfxVmt_runNonDestructiveInversionTest(IfxVmt_MbistSel mbistSel, uint8 rangeSel, uint8 rangeAddrUp, uint8 rangeAddrLow, uint16 *errorAddr)
{
    ...
    /* Run the tests */
    uint32 mcontrolMask = 0x0008;

    mc->MCONTROL.U = mcontrolMask | (1 << IFX_VMT_MC_MCONTROL_START_OFF);
    mc->MCONTROL.U = mcontrolMask | (0 << IFX_VMT_MC_MCONTROL_START_OFF);
...
#endif /* #if IFXVMT_KX_INIT */


#if IFXVMT_K3_PREPARE_EXTRACT_STORE
