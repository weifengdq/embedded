    ...
    if (status == IfxSdmmc_Status_success)
    {
        ...
        if (status == IfxSdmmc_Status_success)
        {
            ...
            if (sd->presetMode == TRUE)
            {
                ...
                /*Clock needs to be disabled completely before reconfiguration.
                 * So delay is needed here.*/
                IfxStm_wait(IFXSDMMC_SD_CLOCK_DISABLE_DELAY); //6us
...
IfxSdmmc_Status IfxSdmmc_Sd_initModule(IfxSdmmc_Sd *sd, IfxSdmmc_Sd_Config *config)
{
    ...
    if (config->interruptConfig.priority > 0)
    {
        ...
        IfxSrc_init(srcSFR, config->interruptConfig.provider, config->interruptConfig.priority, config->interruptConfig.vmId);
        IfxSrc_enable(srcSFR);
    }

    /* initilaise the wakeup card detection interrupt */
    if (config->dmaInterruptConfig.priority > 0)
    {
        /* initialise service request node */
        volatile Ifx_SRC_SRCR *srcSFR;
        srcSFR = IfxSdmmc_getSrcPointer(sdmmcSFR, IfxSdmmc_InterruptType_dma);
        IfxSrc_init(srcSFR, config->dmaInterruptConfig.provider, config->dmaInterruptConfig.priority, config->dmaInterruptConfig.vmId);
        IfxSrc_enable(srcSFR);
    }
...
void IfxSdmmc_Sd_setupPins(IfxSdmmc_Sd *sd, IfxSdmmc_Sd_Pins *pins)
{
    ...
    if (clk != NULL_PTR)
    {
        IfxPort_setPinModeOutput(clk->pin.port, clk->pin.pinIndex, IfxPort_OutputMode_pushPull, clk->select);
...
IfxSdmmc_Status IfxSdmmc_Sd_singleBlockAdma3Transfer(IfxSdmmc_Sd *sd, uint32 *integratedDescrAddress)
{
    IfxSdmmc_Status status  = IfxSdmmc_Status_success;
    uint32          timeout = 0;

    /* set the system address */
    IfxSdmmc_setIntegratedDescriptorAddressForAdma3(sd->sdmmcSFR, (uint32)integratedDescrAddress);
    ...
    if ((timeout == 0) || (IfxSdmmc_isErrorInterruptOccured(sd->sdmmcSFR, IfxSdmmc_ErrorInterrupt_adma) != 0))
    {
        ...
        /* FIXME: implement abort command process */
    }
    else
    {
        IfxSdmmc_clearNormalInterrupt(sd->sdmmcSFR, IfxSdmmc_NormalInterrupt_transferComplete);
...
IfxSdmmc_Status IfxSdmmc_Sd_multiBlockDmaTransfer(IfxSdmmc_Sd *sd, IfxSdmmc_Command command, uint32 address, uint16 blockSize, uint32 *data, IfxSdmmc_TransferDirection direction, uint32 blockCount, IfxSdmmc_BlockBoundarySize blockBoundarySize)
{
    ...
    IfxSdmmc_setBlockSize(sd->sdmmcSFR, blockSize);
    IfxSdmmc_setBlockBoundarySize(sd->sdmmcSFR, blockBoundarySize);
    IfxSdmmc_setMultiBlockSelect(sd->sdmmcSFR, TRUE);
