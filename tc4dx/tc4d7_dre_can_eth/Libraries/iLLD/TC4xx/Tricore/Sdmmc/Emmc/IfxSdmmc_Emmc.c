    ...
    if (status == IfxSdmmc_Status_success)
    {
        ...
        if (status == IfxSdmmc_Status_success)
        {
            ...
            if (cardConfig->speedMode == IfxSdmmc_EmmcSpeedMode_highSpeedSdr)
            {
                ...
                if (status == IfxSdmmc_Status_success)
                {
                    uint32 timeCount = (IFXSDMMC_MMC_DELAY_HIGHSPEED_SHIFT_US * 1000) / IFXSDMMC_1KLOOPDELAY_US;

                    while ((IfxSdmmc_isDataLineBusy(emmc->sdmmcSFR) ||
                            IfxSdmmc_isCommandLineBusy(emmc->sdmmcSFR))
                           && timeCount > 0)
                    {
                        timeCount--;
                    }
...
IfxSdmmc_Status IfxSdmmc_Emmc_singleBlockAdma3Transfer(IfxSdmmc_Emmc *emmc, uint32 *integratedDescrAddress)
{
    IfxSdmmc_Status status  = IfxSdmmc_Status_success;
    uint32          timeout = 0;

    /* set the system address */
    IfxSdmmc_setIntegratedDescriptorAddressForAdma3(emmc->sdmmcSFR, (uint32)integratedDescrAddress);
