...
{
    uint32 result;
    uint32 fPsi5s = IfxClock_getSpbFrequency();
...
boolean IfxPsi5s_Psi5s_initChannel(IfxPsi5s_Psi5s_Channel *channel, const IfxPsi5s_Psi5s_ChannelConfig *config)
{
    ...
    case IfxPsi5s_TriggerType_periodic:
        tempPGC.B.PTE = IfxPsi5s_PeriodicTrigger_enabled;
        tempPGC.B.ETE = IfxPsi5s_ExternalTriggerLine_disabled;
        break;

    case IfxPsi5s_TriggerType_external:
        tempPGC.B.PTE = IfxPsi5s_PeriodicTrigger_disabled;
        tempPGC.B.ETE = IfxPsi5s_ExternalTriggerLine_enabled;
        break;
    }
...
void IfxPsi5s_Psi5s_initChannelConfig(IfxPsi5s_Psi5s_ChannelConfig *config, IfxPsi5s_Psi5s *psi5s)
{
    IfxPsi5s_Psi5s_ChannelConfig IfxPsi5s_Psi5s_defaultChannelConfig = {
        ...
        .pulseGeneration = {
            ...
            .triggersource          = IfxPsi5s_TriggerSelect_gtmTrigger,
        },
        .channelTrigger                     = {
...
#if (IFX_PROT_ENABLED == 1U)
    IfxApProt_setState((Ifx_PROT_PROT *)&psi5sSFR->PROTE, IfxApProt_State_config);
#endif
...
#if (IFX_PROT_ENABLED == 1U)
    IfxApProt_setState((Ifx_PROT_PROT *)&psi5sSFR->PROTE, IfxApProt_State_run);
#endif
...
IFX_STATIC uint32 IfxPsi5s_Psi5s_initializeClock(Ifx_PSI5S *psi5s, const IfxPsi5s_Psi5s_Clock *clock)
{
    ...
    {
        fInput    = IfxClock_getSpbFrequency(); // assumption here is that fBaud2 is equal to fSPB
...
void IfxPsi5s_Psi5s_resetModule(Ifx_PSI5S *psi5s)
{
    ...
    IfxApProt_setState((Ifx_PROT_PROT *)&psi5s->PROTE, IfxApProt_State_config);
#endif
    psi5s->RST.CTRLB.B.KRST = 1;     /* Only if both Kernel reset bits are set a reset is executed */
    psi5s->RST.CTRLA.B.KRST = 1;

    while (psi5s->RST.STAT.B.KRST == 0)
    {
...
IFX_STATIC uint32 IfxPsi5s_Psi5s_setBaudrate(Ifx_PSI5S *psi5s, uint32 baudrate, IfxPsi5s_Psi5s_AscConfig *ascConfig)
{
    ...
    if (ascConfig->receiveMode == IfxPsi5s_AscMode_sync)
    {
        ...
        fInput  = 2 * IfxClock_getSpbFrequency();
