...
#define LPUART_UART_IP_SW_MAJOR_VERSION_C               6
...
static void Lpuart_Uart_Ip_SetUp_Receiver(LPUART_Type * Base);
                                                   ...
                                                   const uint32 TxSize)
{
    ...
    if (!IsReturn)
    {
        ...
        {
            LPUART_UART_IP_DEV_ASSERT(255U != UartUserCfg->TxDMAChannel);
...
Lpuart_Uart_Ip_StatusType Lpuart_Uart_Ip_SyncReceive(const uint8 Instance, uint8 *RxBuff, const uint32 RxSize, const uint32 Timeout)
{
    ...
    if (!IsReturn)
    {
        ...
        /* Flush the Rx buffer, disable receive data full interrupt and set LPUART receiver.*/
        Lpuart_Uart_Ip_SetUp_Receiver(Base);
...
Lpuart_Uart_Ip_StatusType Lpuart_Uart_Ip_AsyncReceive(const uint8 Instance, uint8 * RxBuff, const uint32 RxSize)
{
    ...
    if (!IsReturn)
    {
        ...
        {
            LPUART_UART_IP_DEV_ASSERT(255U != UartUserCfg->RxDMAChannel);
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Lpuart_Uart_Ip_SetUp_Receiver
 * Description   : Flush the Rx buffer, disable receive data full interrupt and set LPUART receiver.
 * This is not a public API as it is called from other driver functions.
 *
 *END**************************************************************************/
static void Lpuart_Uart_Ip_SetUp_Receiver(LPUART_Type * Base)
