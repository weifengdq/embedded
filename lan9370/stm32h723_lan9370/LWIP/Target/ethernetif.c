...
static void low_level_init(struct netif *netif)
{
  ...
  /* Start ETH MAC/DMA (TX + RX). Must happen after RX pool init. */
...
/* USER CODE END low_level_init Code 1 for User BSP */
  ...
  {
    debug_printf("[ETH] HAL_ETH_Init failed! status=%d error=0x%lx DMAMR=0x%lx RCC_AHB1ENR=0x%lx SYSCFG_PMCR=0x%lx RCC_CFGR=0x%lx\r\n",
                 (int)hal_eth_init_status, (unsigned long)heth.ErrorCode,
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  for(q = p; q != NULL; q = q->next)
  {
    ...
    /* Flush D-Cache for this payload buffer before DMA reads it */
...
{
    /* ETH is already started in low_level_init.
     * Always report link UP since LAN9370 Port5 is not a PHY. */
